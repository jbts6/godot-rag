...
The file descriptor looks like `[gd_scene format=3 uid="uid://cecaux1sm7mo0"]` and should be the first entry in the file. Note that scenes saved before Godot 4.6 will also have a `load_steps=` attribute in the file descriptor. This attribute is now deprecated and should be ignored if present.
...
The `class_AnimationPlayer` node works with one or more animation libraries stored in `class_AnimationLibrary` resources. An animation library is a collection of individual `class_Animation` resources, whose structure is documented `here `.
