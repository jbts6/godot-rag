...
Platform ports are designed to be as self-contained as possible. Most of the code can be kept within a single folder located in `platform/`. Like `doc_custom_modules_in_cpp`, this allows for streamlining the build process by making it possible to `git clone` a platform folder within a Godot repository clone's `platform/` folder, then run `scons platform=`. No other steps are necessary for building, unless third-party platform-specific dependencies need to be installed first.
