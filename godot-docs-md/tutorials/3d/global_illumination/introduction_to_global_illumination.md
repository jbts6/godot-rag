...
- For desktop games, it's a good idea to start with `SDFGI ` first as it requires the least amount of setup. Move to other GI techniques later if needed. To improve performance on low-end GPUs and integrated graphics, consider adding an option to disable SDFGI or `VoxelGI
...
- For mobile games, `LightmapGI ` and `ReflectionProbes ` are the only supported options. See also `doc_introduction_to_global_illumination_alternatives`.
