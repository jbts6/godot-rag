...
- By default, rendering in 2D is no longer performed in HDR, which means "overbright" modulate values have no visible effect. Since Godot 4.2, you can enable the project setting `HDR 2D` to perform 2D rendering in HDR. See also `doc_environment_and_post_processing_using_glow_in_2d`.
...
The [editor/project_upgrade/renames_map_3_to_4.cpp](https://github.com/godotengine/godot/blob/master/editor/project_upgrade/renames_map_3_to_4.cpp) source file lists all automatic renames performed by the project upgrade tool. Lines that are commented out refer to API renames that `cannot be performed automatically `.
