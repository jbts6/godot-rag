...
- For 2D, the `class_World2D` resource (obtainable in the `class_Viewport` and `CanvasItem ` nodes) contains functions to get the *RenderingServer Canvas*, and the *PhysicsServer2D Space*. This allows creating 2D objects directly with the server API and using them.
...
- The `class_VisualInstance3D` class, allows getting the scenario *instance* and *instance base* via the `VisualInstance3D.get_instance() ` and `VisualInstance3D.get_base() ` respectively.
