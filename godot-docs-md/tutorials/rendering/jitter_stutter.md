...
> [!TIP]
> On any Godot project, you can use the `--disable-vsync` `command line argument ` to forcibly disable V-Sync. Since Godot 4.2, `--max-fps ` can also be used to set an FPS limit (`0` is unlimited). These arguments can be used at the same time.
