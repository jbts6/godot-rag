...
Per-instance uniform values can also be set at runtime using `set_instance_shader_parameter ` method on a node that inherits from `class_GeometryInstance3D`:
