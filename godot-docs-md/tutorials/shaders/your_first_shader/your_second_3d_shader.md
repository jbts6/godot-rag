...
<doc_shading_language>` doc and the `Spatial Shaders ` doc. Also look at more advanced tutorials in the `Shading section
<toc-learn-features-shading>` and the `3D ` sections.