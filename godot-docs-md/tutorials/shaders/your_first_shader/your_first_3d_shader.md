...
There are multiple node types that you can use to draw a mesh. The main one is `MeshInstance3D `, but you can also use `GPUParticles3D
<class_GPUParticles3D>`, `MultiMeshes ` (with a `MultiMeshInstance3D `), or others.
