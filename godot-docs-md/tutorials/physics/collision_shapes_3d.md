...
You can define the shape of a `class_PhysicsBody3D` by adding one or more `CollisionShape3Ds ` as *direct* child nodes. Indirect child nodes (i.e. children of child nodes) will be ignored and won't be used as collision shapes. Also, note that you must add a `class_Shape3D` *resource* to collision shape nodes in the Inspector dock.
