...
Many of the actions performed in the text editor can also be executed using shortcuts. The actions show their corresponding shortcuts next to them. Within Godot itself, you can find and rebind all shortcuts by going to `Editor > Editor Settings... > Shortcuts`.
...
Below the menus, on the left panel, you will see a list of opened files and documentation pages. Depending on the file type, this list will have an icon next to the file name. For example, the ![script](img/script_editor_icons/Script.webp) icon means that it is a GDScript. the ![scriptcsharp](img/script_editor_icons/ScriptCSharp.webp) means it is a C# script. The ![documentation](img/script_editor_icons/Documentation.webp) means that this is a built-in class reference. Finally, the ![toolscript](img/script_editor_icons/ToolScript.webp) means it is a currently running script (See `tool annotation ` for more on this). Hovering a file will show a tooltip with its relative location in the project folder.
...
- **Run**: If the script extends `EditorScript ` and intended to be executed without running the project, this option runs the script. See `doc_running_code_in_the_editor_editorscript` for more.
...
> [!NOTE]
> This section will only cover the basics of the coding area in terms of the user interface. To learn more about scripting in Godot, refer to the `doc_gdscript` or `Scripting ` documentation.
