...
If you want controller buttons to send echo events, you will have to generate `class_InputEvent` objects by code and parse them using `Input.parse_input_event() ` at regular intervals. This can be accomplished with the help of a `class_Timer` node.
