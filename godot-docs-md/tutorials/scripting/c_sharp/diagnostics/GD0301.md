...
When a generic type parameter is annotated with the `[MustBeVariant]` attribute, the generic type is required to be a Variant-compatible type. For example, the generic `Godot.Collections.Array` type only supports items of a type that can be converted to Variant.
