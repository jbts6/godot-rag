...
The generic `Godot.Collections.Array` type allows restricting the element type to a `Variant-compatible type `.
...
> [!NOTE]
> Despite the name, Godot arrays are more similar to the C# collection `List` than `System.Array`. Their size is not fixed and can grow or shrink as elements are added/removed from the collection.
...
The generic `Godot.Collections.Dictionary` type allows restricting the key and value types to a `Variant-compatible type `.
