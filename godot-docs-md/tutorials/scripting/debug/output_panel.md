...
- `print_tree_pretty() `: Same as `print_tree()`, but with Unicode characters for a more tree-like appearance. This relies on [box-drawing characters](https://en.wikipedia.org/wiki/Box-drawing_characters), so it may not render correctly with all fonts.
