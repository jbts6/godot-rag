...
A custom logger can be registered by creating a class that inherits from `class_logger`, then passing an instance of this class to `OS.add_logger `, in a script's `_init() ` method. A good place to do this is an `autoload `.
