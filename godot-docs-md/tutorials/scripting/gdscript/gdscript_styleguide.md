...
Follow with the optional `@icon` then the `class_name` if necessary. You can turn a GDScript file into a global type in your project using `class_name`. For more information, see `doc_gdscript_basics_class_name`. If the class is meant to be an `abstract class `, add `@abstract` *before* the `class_name` keyword.
