...
Note that the 3D workspace doesn't feature as many tools as the 2D workspace. You'll need external programs or add-ons to edit terrains, animate complex characters, and so on. Godot provides a complete API to extend the editor's functionality using game code. See [The Godot editor is a Godot game](#the-godot-editor-is-a-godot-game) below.
...
<img src="img/introduction_rpg_in_a_box.webp" alt="RPG in a Box is a voxel RPG editor made with Godot. It uses Godot&#39;s UI tools for its node-based programming system and for the rest of the interface." />
