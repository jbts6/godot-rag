...
`void (No return value.)` **area_add_shape**(area: `RID`, shape: `RID`, transform: `Transform3D` = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0), disabled: `bool` = false)

Adds a shape to the area, along with a transform matrix. Shapes are usually referenced by their index, so you should track which shape has a given index.

`void (No return value.)` **area_attach_object_instance_id**(area: `RID`, id: `int`)

Assigns the area to a descendant of `Object`, so it can exist in the node tree.

`void (No return value.)` **area_clear_shapes**(area: `RID`)
...
`void (No return value.)` **area_remove_shape**(area: `RID`, shape_idx: `int`)

Removes a shape from an area. It does not delete the shape, so it can be reassigned later.

`void (No return value.)` **area_set_area_monitor_callback**(area: `RID`, callback: `Callable`)
...
`void (No return value.)` **area_set_collision_layer**(area: `RID`, layer: `int`)

Assigns the area to one or many physics layers.

`void (No return value.)` **area_set_collision_mask**(area: `RID`, mask: `int`)

Sets which physics layers the area will monitor.

`void (No return value.)` **area_set_monitor_callback**(area: `RID`, callback: `Callable`)
...
`void (No return value.)` **area_set_monitorable**(area: `RID`, monitorable: `bool`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **area_set_param**(area: `RID`, param: `AreaParameter`, value: `Variant`)

Sets the value for an area parameter. A list of available parameters is on the `AreaParameter` constants.

`void (No return value.)` **area_set_ray_pickable**(area: `RID`, enable: `bool`)

Sets object pickable with rays.

`void (No return value.)` **area_set_shape**(area: `RID`, shape_idx: `int`, shape: `RID`)

Substitutes a given area shape by another. The old shape is selected by its index, the new one by its `RID`.

`void (No return value.)` **area_set_shape_disabled**(area: `RID`, shape_idx: `int`, disabled: `bool`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **area_set_shape_transform**(area: `RID`, shape_idx: `int`, transform: `Transform3D`)

Sets the transform matrix for an area shape.

`void (No return value.)` **area_set_space**(area: `RID`, space: `RID`)

Assigns a space to the area.

`void (No return value.)` **area_set_transform**(area: `RID`, transform: `Transform3D`)

Sets the transform matrix for an area.

`void (No return value.)` **body_add_collision_exception**(body: `RID`, excepted_body: `RID`)

Adds a body to the list of bodies exempt from collisions.

`void (No return value.)` **body_add_constant_central_force**(body: `RID`, force: `Vector3`)
...
`void (No return value.)` **body_add_constant_force**(body: `RID`, force: `Vector3`, position: `Vector3` = Vector3(0, 0, 0))
...
`void (No return value.)` **body_add_constant_torque**(body: `RID`, torque: `Vector3`)
...
`void (No return value.)` **body_add_shape**(body: `RID`, shape: `RID`, transform: `Transform3D` = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0), disabled: `bool` = false)

Adds a shape to the body, along with a transform matrix. Shapes are usually referenced by their index, so you should track which shape has a given index.

`void (No return value.)` **body_apply_central_force**(body: `RID`, force: `Vector3`)

Applies a directional force without affecting rotation. A force is time dependent and meant to be applied every physics update.

This is equivalent to using `body_apply_force()` at the body's center of mass.

`void (No return value.)` **body_apply_central_impulse**(body: `RID`, impulse: `Vector3`)
...
`void (No return value.)` **body_apply_force**(body: `RID`, force: `Vector3`, position: `Vector3` = Vector3(0, 0, 0))

Applies a positioned force to the body. A force is time dependent and meant to be applied every physics update.

`position` is the offset from the body origin in global coordinates.

`void (No return value.)` **body_apply_impulse**(body: `RID`, impulse: `Vector3`, position: `Vector3` = Vector3(0, 0, 0))
...
`void (No return value.)` **body_apply_torque**(body: `RID`, torque: `Vector3`)

Applies a rotational force without affecting position. A force is time dependent and meant to be applied every physics update.

`void (No return value.)` **body_apply_torque_impulse**(body: `RID`, impulse: `Vector3`)

Applies a rotational impulse to the body without affecting the position.

An impulse is time-independent! Applying an impulse every frame would result in a framerate-dependent force. For this reason, it should only be used when simulating one-time impacts (use the "\_force" functions otherwise).

`void (No return value.)` **body_attach_object_instance_id**(body: `RID`, id: `int`)

Assigns the area to a descendant of `Object`, so it can exist in the node tree.

`void (No return value.)` **body_clear_shapes**(body: `RID`)
...
`void (No return value.)` **body_remove_collision_exception**(body: `RID`, excepted_body: `RID`)

Removes a body from the list of bodies exempt from collisions.

Continuous collision detection tries to predict where a moving body will collide, instead of moving it and correcting its movement if it collided.

`void (No return value.)` **body_remove_shape**(body: `RID`, shape_idx: `int`)

Removes a shape from a body. The shape is not deleted, so it can be reused afterwards.

`void (No return value.)` **body_reset_mass_properties**(body: `RID`)
...
`void (No return value.)` **body_set_axis_lock**(body: `RID`, axis: `BodyAxis`, lock: `bool`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **body_set_axis_velocity**(body: `RID`, axis_velocity: `Vector3`)

Sets an axis velocity. The velocity in the given vector axis will be set as the given vector length. This is useful for jumping behavior.

`void (No return value.)` **body_set_collision_layer**(body: `RID`, layer: `int`)

Sets the physics layer or layers a body belongs to.

`void (No return value.)` **body_set_collision_mask**(body: `RID`, mask: `int`)

Sets the physics layer or layers a body can collide with.

`void (No return value.)` **body_set_collision_priority**(body: `RID`, priority: `float`)

Sets the body's collision priority.

`void (No return value.)` **body_set_constant_force**(body: `RID`, force: `Vector3`)

Sets the body's total constant positional forces applied during each physics update.

See `body_add_constant_force()` and `body_add_constant_central_force()`.

`void (No return value.)` **body_set_constant_torque**(body: `RID`, torque: `Vector3`)

Sets the body's total constant rotational forces applied during each physics update.

See `body_add_constant_torque()`.

`void (No return value.)` **body_set_enable_continuous_collision_detection**(body: `RID`, enable: `bool`)

If `true`, the continuous collision detection mode is enabled.

Continuous collision detection tries to predict where a moving body will collide, instead of moving it and correcting its movement if it collided.

`void (No return value.)` **body_set_force_integration_callback**(body: `RID`, callable: `Callable`, userdata: `Variant` = null)
...
`void (No return value.)` **body_set_max_contacts_reported**(body: `RID`, amount: `int`)

Sets the maximum contacts to report. Bodies can keep a log of the contacts with other bodies. This is enabled by setting the maximum number of contacts reported to a number greater than 0.

`void (No return value.)` **body_set_mode**(body: `RID`, mode: `BodyMode`)

Sets the body mode.

`void (No return value.)` **body_set_omit_force_integration**(body: `RID`, enable: `bool`)
...
`void (No return value.)` **body_set_param**(body: `RID`, param: `BodyParameter`, value: `Variant`)

Sets a body parameter. A list of available parameters is on the `BodyParameter` constants.

`void (No return value.)` **body_set_ray_pickable**(body: `RID`, enable: `bool`)

Sets the body pickable with rays if `enable` is set.

`void (No return value.)` **body_set_shape**(body: `RID`, shape_idx: `int`, shape: `RID`)

Substitutes a given body shape by another. The old shape is selected by its index, the new one by its `RID`.

`void (No return value.)` **body_set_shape_disabled**(body: `RID`, shape_idx: `int`, disabled: `bool`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **body_set_shape_transform**(body: `RID`, shape_idx: `int`, transform: `Transform3D`)

Sets the transform matrix for a body shape.

`void (No return value.)` **body_set_space**(body: `RID`, space: `RID`)

Assigns a space to the body (see `space_create()`).

`void (No return value.)` **body_set_state**(body: `RID`, state: `BodyState`, value: `Variant`)

Sets a body state.

`void (No return value.)` **body_set_state_sync_callback**(body: `RID`, callable: `Callable`)
...
`void (No return value.)` **cone_twist_joint_set_param**(joint: `RID`, param: `ConeTwistJointParam`, value: `float`)
...
`void (No return value.)` **generic_6dof_joint_set_flag**(joint: `RID`, axis: `Axis`, flag: `G6DOFJointAxisFlag`, enable: `bool`)

Sets the value of a given generic 6DOF joint flag.

`void (No return value.)` **generic_6dof_joint_set_param**(joint: `RID`, axis: `Axis`, param: `G6DOFJointAxisParam`, value: `float`)
...
`void (No return value.)` **hinge_joint_set_flag**(joint: `RID`, flag: `HingeJointFlag`, enabled: `bool`)

Sets a hinge joint flag.

`void (No return value.)` **hinge_joint_set_param**(joint: `RID`, param: `HingeJointParam`, value: `float`)

Sets a hinge joint parameter.

`void (No return value.)` **joint_clear**(joint: `RID`)
...
`void (No return value.)` **joint_disable_collisions_between_bodies**(joint: `RID`, disable: `bool`)
...
`void (No return value.)` **joint_make_cone_twist**(joint: `RID`, body_A: `RID`, local_ref_A: `Transform3D`, body_B: `RID`, local_ref_B: `Transform3D`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **joint_make_generic_6dof**(joint: `RID`, body_A: `RID`, local_ref_A: `Transform3D`, body_B: `RID`, local_ref_B: `Transform3D`)
...
`void (No return value.)` **joint_make_hinge**(joint: `RID`, body_A: `RID`, hinge_A: `Transform3D`, body_B: `RID`, hinge_B: `Transform3D`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **joint_make_pin**(joint: `RID`, body_A: `RID`, local_A: `Vector3`, body_B: `RID`, local_B: `Vector3`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **joint_make_slider**(joint: `RID`, body_A: `RID`, local_ref_A: `Transform3D`, body_B: `RID`, local_ref_B: `Transform3D`)

There is currently no description for this method. Please help us by [contributing one](https://contributing.godotengine.org/en/latest/documentation/class_reference.html)!

`void (No return value.)` **joint_set_solver_priority**(joint: `RID`, priority: `int`)
...
`void (No return value.)` **pin_joint_set_local_a**(joint: `RID`, local_A: `Vector3`)

Sets position of the joint in the local space of body a of the joint.

`void (No return value.)` **pin_joint_set_local_b**(joint: `RID`, local_B: `Vector3`)

Sets position of the joint in the local space of body b of the joint.

`void (No return value.)` **pin_joint_set_param**(joint: `RID`, param: `PinJointParam`, value: `float`)
...
`void (No return value.)` **set_active**(active: `bool`)
...
`void (No return value.)` **shape_set_data**(shape: `RID`, data: `Variant`)
...
- `SHAPE_HEIGHTMAP`: a dictionary containing the keys `"width"` and `"depth"` with `int` values, and the key `"heights"` with a value that is a packed array of `float`s of length `width * depth` (that is a `PackedFloat32Array`, or a `PackedFloat64Array` if Godot was compiled with the `precision=double` option), and optionally the keys `"min_height"` and `"max_height"` with `float` values,
...
`void (No return value.)` **shape_set_margin**(shape: `RID`, margin: `float`)
...
`void (No return value.)` **slider_joint_set_param**(joint: `RID`, param: `SliderJointParam`, value: `float`)

Gets a slider joint parameter.

`void (No return value.)` **soft_body_add_collision_exception**(body: `RID`, body_b: `RID`)

Adds the given body to the list of bodies exempt from collisions.

`void (No return value.)` **soft_body_apply_central_force**(body: `RID`, force: `Vector3`)

Distributes and applies a force to all points. A force is time dependent and meant to be applied every physics update.

`void (No return value.)` **soft_body_apply_central_impulse**(body: `RID`, impulse: `Vector3`)

Distributes and applies an impulse to all points.

An impulse is time-independent! Applying an impulse every frame would result in a framerate-dependent force. For this reason, it should only be used when simulating one-time impacts (use the "\_force" functions otherwise).

`void (No return value.)` **soft_body_apply_point_force**(body: `RID`, point_index: `int`, force: `Vector3`)

Applies a force to a point. A force is time dependent and meant to be applied every physics update.

`void (No return value.)` **soft_body_apply_point_impulse**(body: `RID`, point_index: `int`, impulse: `Vector3`)
...
`void (No return value.)` **soft_body_move_point**(body: `RID`, point_index: `int`, global_position: `Vector3`)

Moves the given soft body point to a position in global coordinates.

`void (No return value.)` **soft_body_pin_point**(body: `RID`, point_index: `int`, pin: `bool`)
...
`void (No return value.)` **soft_body_remove_all_pinned_points**(body: `RID`)

Unpins all points of the given soft body.

`void (No return value.)` **soft_body_remove_collision_exception**(body: `RID`, body_b: `RID`)

Removes the given body from the list of bodies exempt from collisions.

`void (No return value.)` **soft_body_set_collision_layer**(body: `RID`, layer: `int`)

Sets the physics layer or layers the given soft body belongs to.

`void (No return value.)` **soft_body_set_collision_mask**(body: `RID`, mask: `int`)

Sets the physics layer or layers the given soft body can collide with.

`void (No return value.)` **soft_body_set_damping_coefficient**(body: `RID`, damping_coefficient: `float`)
...
`void (No return value.)` **soft_body_set_drag_coefficient**(body: `RID`, drag_coefficient: `float`)

Sets the drag coefficient of the given soft body. Higher values increase this body's air resistance.

**Note:** This value is currently unused by Godot's default physics implementation.

`void (No return value.)` **soft_body_set_linear_stiffness**(body: `RID`, stiffness: `float`)
...
`void (No return value.)` **soft_body_set_mesh**(body: `RID`, mesh: `RID`)

Sets the mesh of the given soft body.

`void (No return value.)` **soft_body_set_pressure_coefficient**(body: `RID`, pressure_coefficient: `float`)
...
`void (No return value.)` **soft_body_set_ray_pickable**(body: `RID`, enable: `bool`)

Sets whether the given soft body will be pickable when using object picking.

`void (No return value.)` **soft_body_set_shrinking_factor**(body: `RID`, shrinking_factor: `float`)

Sets the shrinking factor of the given soft body.

`void (No return value.)` **soft_body_set_simulation_precision**(body: `RID`, simulation_precision: `int`)
...
`void (No return value.)` **soft_body_set_space**(body: `RID`, space: `RID`)

Assigns a space to the given soft body (see `space_create()`).

`void (No return value.)` **soft_body_set_state**(body: `RID`, state: `BodyState`, variant: `Variant`)

Sets the given body state for the given body.

**Note:** Godot's default physics implementation does not support `BODY_STATE_LINEAR_VELOCITY`, `BODY_STATE_ANGULAR_VELOCITY`, `BODY_STATE_SLEEPING`, or `BODY_STATE_CAN_SLEEP`.

`void (No return value.)` **soft_body_set_total_mass**(body: `RID`, total_mass: `float`)

Sets the total mass for the given soft body.

`void (No return value.)` **soft_body_set_transform**(body: `RID`, transform: `Transform3D`)

Sets the global transform of the given soft body.

`void (No return value.)` **soft_body_update_rendering_server**(body: `RID`, rendering_server_handler: `PhysicsServer3DRenderingServerHandler`)
...
`void (No return value.)` **space_set_active**(space: `RID`, active: `bool`)

Marks a space as active. It will not have an effect, unless it is assigned to an area or body.

`void (No return value.)` **space_set_param**(space: `RID`, param: `SpaceParameter`, value: `float`)
