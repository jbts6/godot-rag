...
`void (No return value.)` **create**(size: `Vector2i`)

Creates a bitmap with the specified size, filled with `false`.

`void (No return value.)` **create_from_image_alpha**(image: `Image`, threshold: `float` = 0.1)
...
`void (No return value.)` **grow_mask**(pixels: `int`, rect: `Rect2i`)
...
`void (No return value.)` **resize**(new_size: `Vector2i`)

Resizes the image to `new_size`.

`void (No return value.)` **set_bit**(x: `int`, y: `int`, bit: `bool`)

Sets the bitmap's element at the specified position, to the specified value.

`void (No return value.)` **set_bit_rect**(rect: `Rect2i`, bit: `bool`)

Sets a rectangular portion of the bitmap to the specified value.

`void (No return value.)` **set_bitv**(position: `Vector2i`, bit: `bool`)
