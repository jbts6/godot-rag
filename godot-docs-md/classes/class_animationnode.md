...
- `void (No return value.)` **set_filter_enabled**(value: `bool`)
...
`String` **\_get_caption**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

When inheriting from `AnimationRootNode`, implement this virtual method to override the text caption for this animation node.

`AnimationNode` **\_get_child_by_name**(name: `StringName`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`

When inheriting from `AnimationRootNode`, implement this virtual method to return a child animation node by its `name`.

`Dictionary` **\_get_child_nodes**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`Variant` **\_get_parameter_default_value**(parameter: `StringName`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`Array` **\_get_parameter_list**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`bool` **\_has_filter**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

When inheriting from `AnimationRootNode`, implement this virtual method to return whether the blend tree editor should display filter editing on this animation node.

`bool` **\_is_parameter_read_only**(parameter: `StringName`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`float` **\_process**(time: `float`, seek: `bool`, is_external_seeking: `bool`, test_only: `bool`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **blend_animation**(animation: `StringName`, time: `float`, delta: `float`, seeked: `bool`, is_external_seeking: `bool`, blend: `float`, looped_flag: `LoopedFlag` = 0)
...
`void (No return value.)` **remove_input**(index: `int`)

Removes an input, call this only when inactive.

`void (No return value.)` **set_filter_path**(path: `NodePath`, enable: `bool`)
...
`void (No return value.)` **set_parameter**(name: `StringName`, value: `Variant`)
