...
`void (No return value.)` **execute_modifications**(delta: `float`, execution_mode: `int`)
...
`void (No return value.)` **set_bone_local_pose_override**(bone_idx: `int`, override_pose: `Transform2D`, strength: `float`, persistent: `bool`)
...
`void (No return value.)` **set_modification_stack**(modification_stack: `SkeletonModificationStack2D`)
