...
**@export_custom**(hint: `PropertyHint`, hint_string: `String`, usage: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`PropertyUsageFlags`\] = 6)
...
`void (No return value.)` **assert**(condition: `bool`, message: `String` = "")
...
**Note:** Files have to be imported into the engine first to load them using this function. If you want to load `Image`s at run-time, you may use `Image.load()`. If you want to import audio files, you can use the snippet described in `AudioStreamMP3.data`.
...
`void (No return value.)` **print_debug**(...) `vararg`
...
`void (No return value.)` **print_stack**()
