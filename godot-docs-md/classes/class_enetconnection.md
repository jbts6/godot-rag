...
`void (No return value.)` **bandwidth_limit**(in_bandwidth: `int` = 0, out_bandwidth: `int` = 0)

Adjusts the bandwidth limits of a host.

`void (No return value.)` **broadcast**(channel: `int`, packet: `PackedByteArray`, flags: `int`)
...
`void (No return value.)` **channel_limit**(limit: `int`)

Limits the maximum allowed channels of future incoming connections.

`void (No return value.)` **compress**(mode: `CompressionMode`)
...
`void (No return value.)` **refuse_new_connections**(refuse: `bool`)
...
`void (No return value.)` **socket_send**(destination_address: `String`, destination_port: `int`, packet: `PackedByteArray`)
