...
- `void (No return value.)` **set_gravity**(value: `Vector2`)
- `Vector2` **get_gravity**()
...
- `void (No return value.)` **set_jiggle_data_chain_length**(value: `int`)
...
- `void (No return value.)` **set_stiffness**(value: `float`)
- `float` **get_stiffness**()
...
- `void (No return value.)` **set_use_gravity**(value: `bool`)
...
`void (No return value.)` **set_collision_mask**(collision_mask: `int`)

Sets the collision mask that the Jiggle modifier will use when reacting to colliders, if the Jiggle modifier is set to take colliders into account.

`void (No return value.)` **set_jiggle_joint_bone2d_node**(joint_idx: `int`, bone2d_node: `NodePath`)

Sets the `Bone2D` node assigned to the Jiggle joint at `joint_idx`.

`void (No return value.)` **set_jiggle_joint_bone_index**(joint_idx: `int`, bone_idx: `int`)
...
`void (No return value.)` **set_jiggle_joint_damping**(joint_idx: `int`, damping: `float`)

Sets the amount of damping of the Jiggle joint at `joint_idx`.

`void (No return value.)` **set_jiggle_joint_gravity**(joint_idx: `int`, gravity: `Vector2`)

Sets the gravity vector of the Jiggle joint at `joint_idx`.

`void (No return value.)` **set_jiggle_joint_mass**(joint_idx: `int`, mass: `float`)

Sets the of mass of the Jiggle joint at `joint_idx`.

`void (No return value.)` **set_jiggle_joint_override**(joint_idx: `int`, override: `bool`)
...
`void (No return value.)` **set_jiggle_joint_stiffness**(joint_idx: `int`, stiffness: `float`)

Sets the of stiffness of the Jiggle joint at `joint_idx`.

`void (No return value.)` **set_jiggle_joint_use_gravity**(joint_idx: `int`, use_gravity: `bool`)

Sets whether the Jiggle joint at `joint_idx` should use gravity.

`void (No return value.)` **set_use_colliders**(use_colliders: `bool`)
