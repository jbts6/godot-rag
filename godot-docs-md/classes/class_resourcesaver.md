...
`void (No return value.)` **add_resource_format_saver**(format_saver: `ResourceFormatSaver`, at_front: `bool` = false)
...
`void (No return value.)` **remove_resource_format_saver**(format_saver: `ResourceFormatSaver`)

Unregisters the given `ResourceFormatSaver`.

`Error` **save**(resource: `Resource`, path: `String` = "", flags: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`SaverFlags`\] = 0)
