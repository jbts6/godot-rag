...
- `void (No return value.)` **set_margins**(value: `Vector2i`)
...
- `void (No return value.)` **set_separation**(value: `Vector2i`)
...
- `void (No return value.)` **set_texture_region_size**(value: `Vector2i`)
...
- `void (No return value.)` **set_use_texture_padding**(value: `bool`)
...
`void (No return value.)` **clear_tiles_outside_texture**()
...
`void (No return value.)` **create_tile**(atlas_coords: `Vector2i`, size: `Vector2i` = Vector2i(1, 1))
...
`void (No return value.)` **move_tile_in_atlas**(atlas_coords: `Vector2i`, new_atlas_coords: `Vector2i` = Vector2i(-1, -1), new_size: `Vector2i` = Vector2i(-1, -1))
...
`void (No return value.)` **remove_alternative_tile**(atlas_coords: `Vector2i`, alternative_tile: `int`)
...
`void (No return value.)` **remove_tile**(atlas_coords: `Vector2i`)

Remove a tile and its alternative at coordinates `atlas_coords`.

`void (No return value.)` **set_alternative_tile_id**(atlas_coords: `Vector2i`, alternative_tile: `int`, new_id: `int`)
...
`void (No return value.)` **set_tile_animation_columns**(atlas_coords: `Vector2i`, frame_columns: `int`)
...
`void (No return value.)` **set_tile_animation_frame_duration**(atlas_coords: `Vector2i`, frame_index: `int`, duration: `float`)

Sets the animation frame `duration` of frame `frame_index` for the tile at coordinates `atlas_coords`.

`void (No return value.)` **set_tile_animation_frames_count**(atlas_coords: `Vector2i`, frames_count: `int`)

Sets how many animation frames the tile at coordinates `atlas_coords` has.

`void (No return value.)` **set_tile_animation_mode**(atlas_coords: `Vector2i`, mode: `TileAnimationMode`)

Sets the tile animation mode of the tile at `atlas_coords` to `mode`. See also `get_tile_animation_mode()`.

`void (No return value.)` **set_tile_animation_separation**(atlas_coords: `Vector2i`, separation: `Vector2i`)

Sets the margin (in grid tiles) between each tile in the animation layout of the tile at coordinates `atlas_coords` has.

`void (No return value.)` **set_tile_animation_speed**(atlas_coords: `Vector2i`, speed: `float`)
