...
`void (No return value.)` **set_default_extension**(extension_class: `StringName`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
