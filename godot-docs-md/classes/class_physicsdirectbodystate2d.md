...
- `void (No return value.)` **set_sleep_state**(value: `bool`)
- `bool` **is_sleeping**()
...
`void (No return value.)` **add_constant_central_force**(force: `Vector2` = Vector2(0, 0))
...
`void (No return value.)` **apply_central_force**(force: `Vector2` = Vector2(0, 0))

Applies a directional force without affecting rotation. A force is time dependent and meant to be applied every physics update.

This is equivalent to using `apply_force()` at the body's center of mass.

`void (No return value.)` **apply_central_impulse**(impulse: `Vector2`)
...
`void (No return value.)` **apply_torque_impulse**(impulse: `float`)
...
`void (No return value.)` **integrate_forces**()

Updates the body's linear and angular velocity by applying gravity and damping for the equivalent of one physics tick.

`void (No return value.)` **set_constant_force**(force: `Vector2`)

Sets the body's total constant positional forces applied during each physics update.

See `add_constant_force()` and `add_constant_central_force()`.

`void (No return value.)` **set_constant_torque**(torque: `float`)
