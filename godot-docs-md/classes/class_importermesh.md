...
`void (No return value.)` **add_blend_shape**(name: `String`)

Adds name for a blend shape that will be added with `add_surface()`. Must be called before surface is added.

`void (No return value.)` **add_surface**(primitive: `PrimitiveType`, arrays: `Array`, blend_shapes: `Array`\[`Array`\] = \[\], lods: `Dictionary` = {}, material: `Material` = null, name: `String` = "", flags: `int` = 0)
...
`ImporterMesh` **from_mesh**(mesh: `Mesh`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **generate_lods**(normal_merge_angle: `float`, normal_split_angle: `float`, bone_transform_array: `Array`)
...
`ImporterMesh` **merge_importer_meshes**(importer_meshes: `Array`\[`ImporterMesh`\], relative_transforms: `Array`\[`Transform3D`\], deduplicate_surfaces: `bool` = true) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **set_blend_shape_mode**(mode: `BlendShapeMode`)

Sets the blend shape mode.

`void (No return value.)` **set_lightmap_size_hint**(size: `Vector2i`)

Sets the size hint of this mesh for lightmap-unwrapping in UV-space.

`void (No return value.)` **set_surface_material**(surface_idx: `int`, material: `Material`)

Sets a `Material` for a given surface. Surface will be rendered using this material.

`void (No return value.)` **set_surface_name**(surface_idx: `int`, name: `String`)
