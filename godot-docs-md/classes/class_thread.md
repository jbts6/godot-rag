...
`bool` **is_main_thread**() `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **set_thread_safety_checks_enabled**(enabled: `bool`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
