...
`PackedFloat32Array` **\_process_animation_node**(playback_info: `PackedFloat64Array`, test_only: `bool`) `virtual (This method should typically be overridden by the user to have any effect.)` `required (This method is required to be overridden when extending its base class.)`
...
`float` **get_remaining_time**(node_info: `PackedFloat32Array`, break_loop: `bool`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`bool` **is_looping**(node_info: `PackedFloat32Array`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
