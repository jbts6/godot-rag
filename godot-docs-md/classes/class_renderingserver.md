...
- `void (No return value.)` **set_render_loop_enabled**(value: `bool`)
...
`void (No return value.)` **call_on_render_thread**(callable: `Callable`)
...
`void (No return value.)` **camera_attributes_set_auto_exposure**(camera_attributes: `RID`, enable: `bool`, min_sensitivity: `float`, max_sensitivity: `float`, speed: `float`, scale: `float`)
...
`void (No return value.)` **camera_attributes_set_dof_blur**(camera_attributes: `RID`, far_enable: `bool`, far_distance: `float`, far_transition: `float`, near_enable: `bool`, near_distance: `float`, near_transition: `float`, amount: `float`)

Sets the parameters to use with the DOF blur effect. These parameters take on the same meaning as their counterparts in `CameraAttributesPractical`.

`void (No return value.)` **camera_attributes_set_dof_blur_bokeh_shape**(shape: `DOFBokehShape`)
...
`void (No return value.)` **camera_attributes_set_dof_blur_quality**(quality: `DOFBlurQuality`, use_jitter: `bool`)
...
`void (No return value.)` **camera_attributes_set_exposure**(camera_attributes: `RID`, multiplier: `float`, normalization: `float`)
...
`void (No return value.)` **camera_set_camera_attributes**(camera: `RID`, effects: `RID`)

Sets the camera_attributes created with `camera_attributes_create()` to the given camera.

`void (No return value.)` **camera_set_compositor**(camera: `RID`, compositor: `RID`)

Sets the compositor used by this camera. Equivalent to `Camera3D.compositor`.

`void (No return value.)` **camera_set_cull_mask**(camera: `RID`, layers: `int`)
...
`void (No return value.)` **camera_set_environment**(camera: `RID`, env: `RID`)

Sets the environment used by this camera. Equivalent to `Camera3D.environment`.

`void (No return value.)` **camera_set_frustum**(camera: `RID`, size: `float`, offset: `Vector2`, z_near: `float`, z_far: `float`)

Sets camera to use frustum projection. This mode allows adjusting the `offset` argument to create "tilted frustum" effects.

`void (No return value.)` **camera_set_orthogonal**(camera: `RID`, size: `float`, z_near: `float`, z_far: `float`)

Sets camera to use orthogonal projection, also known as orthographic projection. Objects remain the same size on the screen no matter how far away they are.

`void (No return value.)` **camera_set_perspective**(camera: `RID`, fovy_degrees: `float`, z_near: `float`, z_far: `float`)

Sets camera to use perspective projection. Objects on the screen becomes smaller when they are far away.

`void (No return value.)` **camera_set_transform**(camera: `RID`, transform: `Transform3D`)

Sets `Transform3D` of camera.

`void (No return value.)` **camera_set_use_vertical_aspect**(camera: `RID`, enable: `bool`)
...
`void (No return value.)` **canvas_item_add_animation_slice**(item: `RID`, animation_length: `float`, slice_begin: `float`, slice_end: `float`, offset: `float` = 0.0)

Subsequent drawing commands will be ignored unless they fall within the specified animation slice. This is a faster way to implement animations that loop on background rather than redrawing constantly.

`void (No return value.)` **canvas_item_add_circle**(item: `RID`, pos: `Vector2`, radius: `float`, color: `Color`, antialiased: `bool` = false)

Draws a circle on the `CanvasItem` pointed to by the `item` `RID`. See also `CanvasItem.draw_circle()`.

`void (No return value.)` **canvas_item_add_clip_ignore**(item: `RID`, ignore: `bool`)
...
`void (No return value.)` **canvas_item_add_ellipse**(item: `RID`, pos: `Vector2`, major: `float`, minor: `float`, color: `Color`, antialiased: `bool` = false)
...
`void (No return value.)` **canvas_item_add_lcd_texture_rect_region**(item: `RID`, rect: `Rect2`, texture: `RID`, src_rect: `Rect2`, modulate: `Color`)

See also `CanvasItem.draw_lcd_texture_rect_region()`.

`void (No return value.)` **canvas_item_add_line**(item: `RID`, from: `Vector2`, to: `Vector2`, color: `Color`, width: `float` = -1.0, antialiased: `bool` = false)

Draws a line on the `CanvasItem` pointed to by the `item` `RID`. See also `CanvasItem.draw_line()`.

`void (No return value.)` **canvas_item_add_mesh**(item: `RID`, mesh: `RID`, transform: `Transform2D` = Transform2D(1, 0, 0, 1, 0, 0), modulate: `Color` = Color(1, 1, 1, 1), texture: `RID` = RID())
...
`void (No return value.)` **canvas_item_add_msdf_texture_rect_region**(item: `RID`, rect: `Rect2`, texture: `RID`, src_rect: `Rect2`, modulate: `Color` = Color(1, 1, 1, 1), outline_size: `int` = 0, px_range: `float` = 1.0, scale: `float` = 1.0)

See also `CanvasItem.draw_msdf_texture_rect_region()`.

`void (No return value.)` **canvas_item_add_multiline**(item: `RID`, points: `PackedVector2Array`, colors: `PackedColorArray`, width: `float` = -1.0, antialiased: `bool` = false)
...
`void (No return value.)` **canvas_item_add_multimesh**(item: `RID`, mesh: `RID`, texture: `RID` = RID())

Draws a 2D `MultiMesh` on the `CanvasItem` pointed to by the `item` `RID`. See also `CanvasItem.draw_multimesh()`.

`void (No return value.)` **canvas_item_add_nine_patch**(item: `RID`, rect: `Rect2`, source: `Rect2`, texture: `RID`, topleft: `Vector2`, bottomright: `Vector2`, x_axis_mode: `NinePatchAxisMode` = 0, y_axis_mode: `NinePatchAxisMode` = 0, draw_center: `bool` = true, modulate: `Color` = Color(1, 1, 1, 1))

Draws a nine-patch rectangle on the `CanvasItem` pointed to by the `item` `RID`.

`void (No return value.)` **canvas_item_add_particles**(item: `RID`, particles: `RID`, texture: `RID`)

Draws particles on the `CanvasItem` pointed to by the `item` `RID`.

`void (No return value.)` **canvas_item_add_polygon**(item: `RID`, points: `PackedVector2Array`, colors: `PackedColorArray`, uvs: `PackedVector2Array` = PackedVector2Array(), texture: `RID` = RID())
...
`void (No return value.)` **canvas_item_add_polyline**(item: `RID`, points: `PackedVector2Array`, colors: `PackedColorArray`, width: `float` = -1.0, antialiased: `bool` = false)
...
`void (No return value.)` **canvas_item_add_primitive**(item: `RID`, points: `PackedVector2Array`, colors: `PackedColorArray`, uvs: `PackedVector2Array`, texture: `RID`)

Draws a 2D primitive on the `CanvasItem` pointed to by the `item` `RID`. See also `CanvasItem.draw_primitive()`.

`void (No return value.)` **canvas_item_add_rect**(item: `RID`, rect: `Rect2`, color: `Color`, antialiased: `bool` = false)

Draws a rectangle on the `CanvasItem` pointed to by the `item` `RID`. See also `CanvasItem.draw_rect()`.

`void (No return value.)` **canvas_item_add_set_transform**(item: `RID`, transform: `Transform2D`)

Sets a `Transform2D` that will be used to transform subsequent canvas item commands.

`void (No return value.)` **canvas_item_add_texture_rect**(item: `RID`, rect: `Rect2`, texture: `RID`, tile: `bool` = false, modulate: `Color` = Color(1, 1, 1, 1), transpose: `bool` = false)
...
`void (No return value.)` **canvas_item_add_texture_rect_region**(item: `RID`, rect: `Rect2`, texture: `RID`, src_rect: `Rect2`, modulate: `Color` = Color(1, 1, 1, 1), transpose: `bool` = false, clip_uv: `bool` = true)
...
`void (No return value.)` **canvas_item_add_triangle_array**(item: `RID`, indices: `PackedInt32Array`, points: `PackedVector2Array`, colors: `PackedColorArray`, uvs: `PackedVector2Array` = PackedVector2Array(), bones: `PackedInt32Array` = PackedInt32Array(), weights: `PackedFloat32Array` = PackedFloat32Array(), texture: `RID` = RID(), count: `int` = -1)
...
`void (No return value.)` **canvas_item_attach_skeleton**(item: `RID`, skeleton: `RID`)

Attaches a skeleton to the `CanvasItem`. Removes the previous skeleton.

`void (No return value.)` **canvas_item_clear**(item: `RID`)
...
`void (No return value.)` **canvas_item_reset_physics_interpolation**(item: `RID`)

Prevents physics interpolation for the current physics tick.

This is useful when moving a canvas item to a new location, to give an instantaneous change rather than interpolation from the previous location.

`void (No return value.)` **canvas_item_set_canvas_group_mode**(item: `RID`, mode: `CanvasGroupMode`, clear_margin: `float` = 5.0, fit_empty: `bool` = false, fit_margin: `float` = 0.0, blur_mipmaps: `bool` = false)
...
`void (No return value.)` **canvas_item_set_clip**(item: `RID`, clip: `bool`)
...
`void (No return value.)` **canvas_item_set_copy_to_backbuffer**(item: `RID`, enabled: `bool`, rect: `Rect2`)

Sets the `CanvasItem` to copy a rect to the backbuffer.

`void (No return value.)` **canvas_item_set_custom_rect**(item: `RID`, use_custom_rect: `bool`, rect: `Rect2` = Rect2(0, 0, 0, 0))
...
`void (No return value.)` **canvas_item_set_default_texture_filter**(item: `RID`, filter: `CanvasItemTextureFilter`)
...
`void (No return value.)` **canvas_item_set_default_texture_repeat**(item: `RID`, repeat: `CanvasItemTextureRepeat`)
...
`void (No return value.)` **canvas_item_set_distance_field_mode**(item: `RID`, enabled: `bool`)
...
`void (No return value.)` **canvas_item_set_draw_behind_parent**(item: `RID`, enabled: `bool`)
...
`void (No return value.)` **canvas_item_set_draw_index**(item: `RID`, index: `int`)

Sets the index for the `CanvasItem`.

`void (No return value.)` **canvas_item_set_instance_shader_parameter**(instance: `RID`, parameter: `StringName`, value: `Variant`)
...
`void (No return value.)` **canvas_item_set_interpolated**(item: `RID`, interpolated: `bool`)

If `interpolated` is `true`, turns on physics interpolation for the canvas item.

`void (No return value.)` **canvas_item_set_light_mask**(item: `RID`, mask: `int`)

Sets the light `mask` for the canvas item specified by the `item` RID. Equivalent to `CanvasItem.light_mask`.

`void (No return value.)` **canvas_item_set_material**(item: `RID`, material: `RID`)

Sets a new `material` to the canvas item specified by the `item` RID. Equivalent to `CanvasItem.material`.

`void (No return value.)` **canvas_item_set_modulate**(item: `RID`, color: `Color`)
...
`void (No return value.)` **canvas_item_set_parent**(item: `RID`, parent: `RID`)
...
`void (No return value.)` **canvas_item_set_self_modulate**(item: `RID`, color: `Color`)
...
`void (No return value.)` **canvas_item_set_sort_children_by_y**(item: `RID`, enabled: `bool`)
...
`void (No return value.)` **canvas_item_set_transform**(item: `RID`, transform: `Transform2D`)
...
`void (No return value.)` **canvas_item_set_use_parent_material**(item: `RID`, enabled: `bool`)

Sets if the `CanvasItem` uses its parent's material.

`void (No return value.)` **canvas_item_set_visibility_layer**(item: `RID`, visibility_layer: `int`)

Sets the rendering visibility layer associated with this `CanvasItem`. Only `Viewport` nodes with a matching rendering mask will render this `CanvasItem`.

`void (No return value.)` **canvas_item_set_visibility_notifier**(item: `RID`, enable: `bool`, area: `Rect2`, enter_callable: `Callable`, exit_callable: `Callable`)
...
`void (No return value.)` **canvas_item_set_visible**(item: `RID`, visible: `bool`)

Sets the visibility of the `CanvasItem`.

`void (No return value.)` **canvas_item_set_z_as_relative_to_parent**(item: `RID`, enabled: `bool`)

If this is enabled, the Z index of the parent will be added to the children's Z index.

`void (No return value.)` **canvas_item_set_z_index**(item: `RID`, z_index: `int`)

Sets the `CanvasItem`'s Z index, i.e. its draw order (lower indexes are drawn first).

`void (No return value.)` **canvas_item_transform_physics_interpolation**(item: `RID`, transform: `Transform2D`)
...
`void (No return value.)` **canvas_light_attach_to_canvas**(light: `RID`, canvas: `RID`)
...
`void (No return value.)` **canvas_light_occluder_attach_to_canvas**(occluder: `RID`, canvas: `RID`)
...
`void (No return value.)` **canvas_light_occluder_reset_physics_interpolation**(occluder: `RID`)

Prevents physics interpolation for the current physics tick.

This is useful when moving an occluder to a new location, to give an instantaneous change rather than interpolation from the previous location.

`void (No return value.)` **canvas_light_occluder_set_as_sdf_collision**(occluder: `RID`, enable: `bool`)

Enables or disables using the light occluder as a signed distance field for 2D particle collision.

`void (No return value.)` **canvas_light_occluder_set_enabled**(occluder: `RID`, enabled: `bool`)

Enables or disables light occluder.

`void (No return value.)` **canvas_light_occluder_set_interpolated**(occluder: `RID`, interpolated: `bool`)

If `interpolated` is `true`, turns on physics interpolation for the light occluder.

`void (No return value.)` **canvas_light_occluder_set_light_mask**(occluder: `RID`, mask: `int`)

The light mask. See `LightOccluder2D` for more information on light masks.

`void (No return value.)` **canvas_light_occluder_set_polygon**(occluder: `RID`, polygon: `RID`)

Sets a light occluder's polygon.

`void (No return value.)` **canvas_light_occluder_set_transform**(occluder: `RID`, transform: `Transform2D`)

Sets a light occluder's `Transform2D`.

`void (No return value.)` **canvas_light_occluder_transform_physics_interpolation**(occluder: `RID`, transform: `Transform2D`)
...
`void (No return value.)` **canvas_light_reset_physics_interpolation**(light: `RID`)

Prevents physics interpolation for the current physics tick.

This is useful when moving a canvas item to a new location, to give an instantaneous change rather than interpolation from the previous location.

`void (No return value.)` **canvas_light_set_blend_mode**(light: `RID`, mode: `CanvasLightBlendMode`)

Sets the blend mode for the given canvas light to `mode`. Equivalent to `Light2D.blend_mode`.

`void (No return value.)` **canvas_light_set_color**(light: `RID`, color: `Color`)

Sets the color for a light.

`void (No return value.)` **canvas_light_set_enabled**(light: `RID`, enabled: `bool`)

Enables or disables a canvas light.

`void (No return value.)` **canvas_light_set_energy**(light: `RID`, energy: `float`)

Sets a canvas light's energy.

`void (No return value.)` **canvas_light_set_height**(light: `RID`, height: `float`)

Sets a canvas light's height.

`void (No return value.)` **canvas_light_set_interpolated**(light: `RID`, interpolated: `bool`)

If `interpolated` is `true`, turns on physics interpolation for the canvas light.

`void (No return value.)` **canvas_light_set_item_cull_mask**(light: `RID`, mask: `int`)

The light mask. See `LightOccluder2D` for more information on light masks.

`void (No return value.)` **canvas_light_set_item_shadow_cull_mask**(light: `RID`, mask: `int`)

The binary mask used to determine which layers this canvas light's shadows affects. See `LightOccluder2D` for more information on light masks.

`void (No return value.)` **canvas_light_set_layer_range**(light: `RID`, min_layer: `int`, max_layer: `int`)

The layer range that gets rendered with this light.

`void (No return value.)` **canvas_light_set_mode**(light: `RID`, mode: `CanvasLightMode`)

Sets the mode of the canvas light.

`void (No return value.)` **canvas_light_set_shadow_color**(light: `RID`, color: `Color`)

Sets the color of the canvas light's shadow.

`void (No return value.)` **canvas_light_set_shadow_enabled**(light: `RID`, enabled: `bool`)

Enables or disables the canvas light's shadow.

`void (No return value.)` **canvas_light_set_shadow_filter**(light: `RID`, filter: `CanvasLightShadowFilter`)

Sets the canvas light's shadow's filter.

`void (No return value.)` **canvas_light_set_shadow_smooth**(light: `RID`, smooth: `float`)

Smoothens the shadow. The lower, the smoother.

`void (No return value.)` **canvas_light_set_texture**(light: `RID`, texture: `RID`)

Sets the texture to be used by a `PointLight2D`. Equivalent to `PointLight2D.texture`.

`void (No return value.)` **canvas_light_set_texture_offset**(light: `RID`, offset: `Vector2`)

Sets the offset of a `PointLight2D`'s texture. Equivalent to `PointLight2D.offset`.

`void (No return value.)` **canvas_light_set_texture_scale**(light: `RID`, scale: `float`)

Sets the scale factor of a `PointLight2D`'s texture. Equivalent to `PointLight2D.texture_scale`.

`void (No return value.)` **canvas_light_set_transform**(light: `RID`, transform: `Transform2D`)

Sets the canvas light's `Transform2D`.

`void (No return value.)` **canvas_light_set_z_range**(light: `RID`, min_z: `int`, max_z: `int`)
...
`void (No return value.)` **canvas_light_transform_physics_interpolation**(light: `RID`, transform: `Transform2D`)
...
`void (No return value.)` **canvas_occluder_polygon_set_cull_mode**(occluder_polygon: `RID`, mode: `CanvasOccluderPolygonCullMode`)

Sets an occluder polygon's cull mode.

`void (No return value.)` **canvas_occluder_polygon_set_shape**(occluder_polygon: `RID`, shape: `PackedVector2Array`, closed: `bool`)

Sets the shape of the occluder polygon.

`void (No return value.)` **canvas_set_disable_scale**(disable: `bool`)
...
`void (No return value.)` **canvas_set_item_mirroring**(canvas: `RID`, item: `RID`, mirroring: `Vector2`)
...
`void (No return value.)` **canvas_set_item_repeat**(item: `RID`, repeat_size: `Vector2`, repeat_times: `int`)
...
`void (No return value.)` **canvas_set_modulate**(canvas: `RID`, color: `Color`)

Modulates all colors in the given canvas.

`void (No return value.)` **canvas_set_shadow_texture_size**(size: `int`)
...
`void (No return value.)` **canvas_texture_set_channel**(canvas_texture: `RID`, channel: `CanvasTextureChannel`, texture: `RID`)
...
`void (No return value.)` **canvas_texture_set_shading_parameters**(canvas_texture: `RID`, base_color: `Color`, shininess: `float`)
...
`void (No return value.)` **canvas_texture_set_texture_filter**(canvas_texture: `RID`, filter: `CanvasItemTextureFilter`)

Sets the texture `filter` mode to use for the canvas texture specified by the `canvas_texture` RID.

`void (No return value.)` **canvas_texture_set_texture_repeat**(canvas_texture: `RID`, repeat: `CanvasItemTextureRepeat`)
...
`void (No return value.)` **compositor_effect_set_callback**(effect: `RID`, callback_type: `CompositorEffectCallbackType`, callback: `Callable`)

Sets the callback type (`callback_type`) and callback method(`callback`) for this rendering effect.

`void (No return value.)` **compositor_effect_set_enabled**(effect: `RID`, enabled: `bool`)

Enables/disables this rendering effect.

`void (No return value.)` **compositor_effect_set_flag**(effect: `RID`, flag: `CompositorEffectFlags`, set: `bool`)

Sets the flag (`flag`) for this rendering effect to `true` or `false` (`set`).

`void (No return value.)` **compositor_set_compositor_effects**(compositor: `RID`, effects: `Array`\[`RID`\])
...
`void (No return value.)` **decal_set_albedo_mix**(decal: `RID`, albedo_mix: `float`)

Sets the `albedo_mix` in the decal specified by the `decal` RID. Equivalent to `Decal.albedo_mix`.

`void (No return value.)` **decal_set_cull_mask**(decal: `RID`, mask: `int`)

Sets the cull `mask` in the decal specified by the `decal` RID. Equivalent to `Decal.cull_mask`.

`void (No return value.)` **decal_set_distance_fade**(decal: `RID`, enabled: `bool`, begin: `float`, length: `float`)
...
`void (No return value.)` **decal_set_emission_energy**(decal: `RID`, energy: `float`)

Sets the emission `energy` in the decal specified by the `decal` RID. Equivalent to `Decal.emission_energy`.

`void (No return value.)` **decal_set_fade**(decal: `RID`, above: `float`, below: `float`)
...
`void (No return value.)` **directional_shadow_atlas_set_size**(size: `int`, is_16bits: `bool`)
...
`void (No return value.)` **directional_soft_shadow_filter_set_quality**(quality: `ShadowQuality`)
...
`void (No return value.)` **environment_glow_set_use_bicubic_upscale**(enable: `bool`)
...
`void (No return value.)` **environment_set_adjustment**(env: `RID`, enable: `bool`, brightness: `float`, contrast: `float`, saturation: `float`, use_1d_color_correction: `bool`, color_correction: `RID`)

Sets the values to be used with the "adjustments" post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_ambient_light**(env: `RID`, color: `Color`, ambient: `EnvironmentAmbientSource` = 0, energy: `float` = 1.0, sky_contribution: `float` = 0.0, reflection_source: `EnvironmentReflectionSource` = 0)

Sets the values to be used for ambient light rendering. See `Environment` for more details.

`void (No return value.)` **environment_set_background**(env: `RID`, bg: `EnvironmentBG`)

Sets the environment's background mode. Equivalent to `Environment.background_mode`.

`void (No return value.)` **environment_set_bg_color**(env: `RID`, color: `Color`)

Color displayed for clear areas of the scene. Only effective if using the `ENV_BG_COLOR` background mode.

`void (No return value.)` **environment_set_bg_energy**(env: `RID`, multiplier: `float`, exposure_value: `float`)

Sets the intensity of the background color.

`void (No return value.)` **environment_set_camera_id**(env: `RID`, id: `int`)

Sets the camera ID to be used as environment background.

`void (No return value.)` **environment_set_canvas_max_layer**(env: `RID`, max_layer: `int`)

Sets the maximum layer to use if using Canvas background mode.

`void (No return value.)` **environment_set_fog**(env: `RID`, enable: `bool`, light_color: `Color`, light_energy: `float`, sun_scatter: `float`, density: `float`, height: `float`, height_density: `float`, aerial_perspective: `float`, sky_affect: `float`, fog_mode: `EnvironmentFogMode` = 0)

Configures fog for the specified environment RID. See `fog_*` properties in `Environment` for more information.

`void (No return value.)` **environment_set_fog_depth**(env: `RID`, curve: `float`, begin: `float`, end: `float`)
...
`void (No return value.)` **environment_set_glow**(env: `RID`, enable: `bool`, levels: `PackedFloat32Array`, intensity: `float`, strength: `float`, mix: `float`, bloom_threshold: `float`, blend_mode: `EnvironmentGlowBlendMode`, hdr_bleed_threshold: `float`, hdr_bleed_scale: `float`, hdr_luminance_cap: `float`, glow_map_strength: `float`, glow_map: `RID`)

Configures glow for the specified environment RID. See `glow_*` properties in `Environment` for more information.

`void (No return value.)` **environment_set_sdfgi**(env: `RID`, enable: `bool`, cascades: `int`, min_cell_size: `float`, y_scale: `EnvironmentSDFGIYScale`, use_occlusion: `bool`, bounce_feedback: `float`, read_sky: `bool`, energy: `float`, normal_bias: `float`, probe_bias: `float`)
...
`void (No return value.)` **environment_set_sdfgi_frames_to_converge**(frames: `EnvironmentSDFGIFramesToConverge`)
...
`void (No return value.)` **environment_set_sdfgi_frames_to_update_light**(frames: `EnvironmentSDFGIFramesToUpdateLight`)
...
`void (No return value.)` **environment_set_sdfgi_ray_count**(ray_count: `EnvironmentSDFGIRayCount`)
...
`void (No return value.)` **environment_set_sky**(env: `RID`, sky: `RID`)

Sets the `Sky` to be used as the environment's background when using *BGMode* sky. Equivalent to `Environment.sky`.

`void (No return value.)` **environment_set_sky_custom_fov**(env: `RID`, scale: `float`)

Sets a custom field of view for the background `Sky`. Equivalent to `Environment.sky_custom_fov`.

`void (No return value.)` **environment_set_sky_orientation**(env: `RID`, orientation: `Basis`)
...
`void (No return value.)` **environment_set_ssao**(env: `RID`, enable: `bool`, radius: `float`, intensity: `float`, power: `float`, detail: `float`, horizon: `float`, sharpness: `float`, light_affect: `float`, ao_channel_affect: `float`)

Sets the variables to be used with the screen-space ambient occlusion (SSAO) post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_ssao_quality**(quality: `EnvironmentSSAOQuality`, half_size: `bool`, adaptive_target: `float`, blur_passes: `int`, fadeout_from: `float`, fadeout_to: `float`)

Sets the quality level of the screen-space ambient occlusion (SSAO) post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_ssil_quality**(quality: `EnvironmentSSILQuality`, half_size: `bool`, adaptive_target: `float`, blur_passes: `int`, fadeout_from: `float`, fadeout_to: `float`)

Sets the quality level of the screen-space indirect lighting (SSIL) post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_ssr**(env: `RID`, enable: `bool`, max_steps: `int`, fade_in: `float`, fade_out: `float`, depth_tolerance: `float`)

Sets the variables to be used with the screen-space reflections (SSR) post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_ssr_half_size**(half_size: `bool`)
...
`void (No return value.)` **environment_set_ssr_roughness_quality**(quality: `EnvironmentSSRRoughnessQuality`)

**Deprecated:** This option no longer does anything.

`void (No return value.)` **environment_set_tonemap**(env: `RID`, tone_mapper: `EnvironmentToneMapper`, exposure: `float`, white: `float`)

Sets the variables to be used with the "tonemap" post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_tonemap_agx_contrast**(env: `RID`, agx_contrast: `float`)

See `Environment.tonemap_agx_contrast` for more details.

`void (No return value.)` **environment_set_volumetric_fog**(env: `RID`, enable: `bool`, density: `float`, albedo: `Color`, emission: `Color`, emission_energy: `float`, anisotropy: `float`, length: `float`, detail_spread: `float`, gi_inject: `float`, temporal_reprojection: `bool`, temporal_reprojection_amount: `float`, ambient_inject: `float`, sky_affect: `float`)

Sets the variables to be used with the volumetric fog post-process effect. See `Environment` for more details.

`void (No return value.)` **environment_set_volumetric_fog_filter_active**(active: `bool`)

Enables filtering of the volumetric fog scattering buffer. This results in much smoother volumes with very few under-sampling artifacts.

`void (No return value.)` **environment_set_volumetric_fog_volume_size**(size: `int`, depth: `int`)
...
`void (No return value.)` **fog_volume_set_material**(fog_volume: `RID`, material: `RID`)

Sets the `Material` of the fog volume. Can be either a `FogMaterial` or a custom `ShaderMaterial`.

`void (No return value.)` **fog_volume_set_shape**(fog_volume: `RID`, shape: `FogVolumeShape`)
...
`void (No return value.)` **fog_volume_set_size**(fog_volume: `RID`, size: `Vector3`)
...
`void (No return value.)` **force_draw**(swap_buffers: `bool` = true, frame_step: `float` = 0.0)

Forces redrawing of all viewports at once. Must be called from the main thread.

`void (No return value.)` **force_sync**()
...
`void (No return value.)` **gi_set_use_half_resolution**(half_resolution: `bool`)
...
`void (No return value.)` **global_shader_parameter_add**(name: `StringName`, type: `GlobalShaderParameterType`, default_value: `Variant`)
...
`void (No return value.)` **global_shader_parameter_remove**(name: `StringName`)

Removes the global shader uniform specified by `name`.

`void (No return value.)` **global_shader_parameter_set**(name: `StringName`, value: `Variant`)

Sets the global shader uniform `name` to `value`.

`void (No return value.)` **global_shader_parameter_set_override**(name: `StringName`, value: `Variant`)
...
`void (No return value.)` **instance_attach_object_instance_id**(instance: `RID`, id: `int`)
...
`void (No return value.)` **instance_attach_skeleton**(instance: `RID`, skeleton: `RID`)
...
`void (No return value.)` **instance_geometry_set_cast_shadows_setting**(instance: `RID`, shadow_casting_setting: `ShadowCastingSetting`)

Sets the shadow casting setting. Equivalent to `GeometryInstance3D.cast_shadow`.

`void (No return value.)` **instance_geometry_set_flag**(instance: `RID`, flag: `InstanceFlags`, enabled: `bool`)

Sets the `flag` for a given `instance` to `enabled`.

`void (No return value.)` **instance_geometry_set_lightmap**(instance: `RID`, lightmap: `RID`, lightmap_uv_scale: `Rect2`, lightmap_slice: `int`)
...
`void (No return value.)` **instance_geometry_set_lod_bias**(instance: `RID`, lod_bias: `float`)
...
`void (No return value.)` **instance_geometry_set_material_overlay**(instance: `RID`, material: `RID`)
...
`void (No return value.)` **instance_geometry_set_material_override**(instance: `RID`, material: `RID`)
...
`void (No return value.)` **instance_geometry_set_shader_parameter**(instance: `RID`, parameter: `StringName`, value: `Variant`)
...
`void (No return value.)` **instance_geometry_set_transparency**(instance: `RID`, transparency: `float`)
...
`void (No return value.)` **instance_geometry_set_visibility_range**(instance: `RID`, min: `float`, max: `float`, min_margin: `float`, max_margin: `float`, fade_mode: `VisibilityRangeFadeMode`)
...
`void (No return value.)` **instance_set_base**(instance: `RID`, base: `RID`)

Sets the base of the instance. A base can be any of the 3D objects that are created in the RenderingServer that can be displayed. For example, any of the light types, mesh, multimesh, particle system, reflection probe, decal, lightmap, voxel GI and visibility notifiers are all types that can be set as the base of an instance in order to be displayed in the scenario.

`void (No return value.)` **instance_set_blend_shape_weight**(instance: `RID`, shape: `int`, weight: `float`)

Sets the weight for a given blend shape associated with this instance.

`void (No return value.)` **instance_set_custom_aabb**(instance: `RID`, aabb: `AABB`)
...
`void (No return value.)` **instance_set_extra_visibility_margin**(instance: `RID`, margin: `float`)
...
`void (No return value.)` **instance_set_ignore_culling**(instance: `RID`, enabled: `bool`)
...
`void (No return value.)` **instance_set_layer_mask**(instance: `RID`, mask: `int`)

Sets the render layers that this instance will be drawn to. Equivalent to `VisualInstance3D.layers`.

`void (No return value.)` **instance_set_pivot_data**(instance: `RID`, sorting_offset: `float`, use_aabb_center: `bool`)

Sets the sorting offset and switches between using the bounding box or instance origin for depth sorting.

`void (No return value.)` **instance_set_scenario**(instance: `RID`, scenario: `RID`)

Sets the scenario that the instance is in. The scenario is the 3D world that the objects will be displayed in.

`void (No return value.)` **instance_set_surface_override_material**(instance: `RID`, surface: `int`, material: `RID`)

Sets the override material of a specific surface. Equivalent to `MeshInstance3D.set_surface_override_material()`.

`void (No return value.)` **instance_set_transform**(instance: `RID`, transform: `Transform3D`)

Sets the world space transform of the instance. Equivalent to `Node3D.global_transform`.

`void (No return value.)` **instance_set_visibility_parent**(instance: `RID`, parent: `RID`)

Sets the visibility parent for the given instance. Equivalent to `Node3D.visibility_parent`.

`void (No return value.)` **instance_set_visible**(instance: `RID`, visible: `bool`)

Sets whether an instance is drawn or not. Equivalent to `Node3D.visible`.

`void (No return value.)` **instance_teleport**(instance: `RID`)
...
`void (No return value.)` **light_area_set_normalize_energy**(light: `RID`, enable: `bool`)
...
`void (No return value.)` **light_area_set_size**(light: `RID`, size: `Vector2`)

Sets the extents (width and height) in meters for this area light. Equivalent to `AreaLight3D.area_size`.

`void (No return value.)` **light_directional_set_blend_splits**(light: `RID`, enable: `bool`)
...
`void (No return value.)` **light_directional_set_shadow_mode**(light: `RID`, mode: `LightDirectionalShadowMode`)

Sets the shadow mode for this directional light. Equivalent to `DirectionalLight3D.directional_shadow_mode`.

`void (No return value.)` **light_directional_set_sky_mode**(light: `RID`, mode: `LightDirectionalSkyMode`)
...
`void (No return value.)` **light_omni_set_shadow_mode**(light: `RID`, mode: `LightOmniShadowMode`)
...
`void (No return value.)` **light_projectors_set_filter**(filter: `LightProjectorFilter`)

Sets the texture filter mode to use when rendering light projectors. This parameter is global and cannot be set on a per-light basis.

`void (No return value.)` **light_set_bake_mode**(light: `RID`, bake_mode: `LightBakeMode`)

Sets the bake mode to use for the specified 3D light. Equivalent to `Light3D.light_bake_mode`.

`void (No return value.)` **light_set_color**(light: `RID`, color: `Color`)

Sets the color of the light. Equivalent to `Light3D.light_color`.

`void (No return value.)` **light_set_cull_mask**(light: `RID`, mask: `int`)
...
`void (No return value.)` **light_set_distance_fade**(decal: `RID`, enabled: `bool`, begin: `float`, shadow: `float`, length: `float`)
...
`void (No return value.)` **light_set_max_sdfgi_cascade**(light: `RID`, cascade: `int`)

Sets the maximum SDFGI cascade in which the 3D light's indirect lighting is rendered. Higher values allow the light to be rendered in SDFGI further away from the camera.

`void (No return value.)` **light_set_negative**(light: `RID`, enable: `bool`)

If `true`, the 3D light will subtract light instead of adding light. Equivalent to `Light3D.light_negative`.

`void (No return value.)` **light_set_param**(light: `RID`, param: `LightParam`, value: `float`)

Sets the specified 3D light parameter. Equivalent to `Light3D.set_param()`.

`void (No return value.)` **light_set_projector**(light: `RID`, texture: `RID`)

Sets the projector texture to use for the specified 3D light. Equivalent to `Light3D.light_projector`.

`void (No return value.)` **light_set_reverse_cull_face_mode**(light: `RID`, enabled: `bool`)
...
`void (No return value.)` **light_set_shadow**(light: `RID`, enabled: `bool`)

If `true`, light will cast shadows. Equivalent to `Light3D.shadow_enabled`.

`void (No return value.)` **light_set_shadow_caster_mask**(light: `RID`, mask: `int`)
...
`void (No return value.)` **lightmap_set_baked_exposure_normalization**(lightmap: `RID`, baked_exposure: `float`)
...
`void (No return value.)` **lightmap_set_probe_bounds**(lightmap: `RID`, bounds: `AABB`)
...
`void (No return value.)` **lightmap_set_probe_capture_data**(lightmap: `RID`, points: `PackedVector3Array`, point_sh: `PackedColorArray`, tetrahedra: `PackedInt32Array`, bsp_tree: `PackedInt32Array`)
...
`void (No return value.)` **lightmap_set_probe_capture_update_speed**(speed: `float`)
...
`void (No return value.)` **lightmap_set_probe_interior**(lightmap: `RID`, interior: `bool`)
...
`void (No return value.)` **lightmap_set_textures**(lightmap: `RID`, light: `RID`, uses_sh: `bool`)
...
`void (No return value.)` **lightmaps_set_bicubic_filter**(enable: `bool`)
...
`void (No return value.)` **material_set_next_pass**(material: `RID`, next_material: `RID`)

Sets an object's next material.

`void (No return value.)` **material_set_param**(material: `RID`, parameter: `StringName`, value: `Variant`)

Sets a material's parameter.

`void (No return value.)` **material_set_render_priority**(material: `RID`, priority: `int`)

Sets a material's render priority.

`void (No return value.)` **material_set_shader**(shader_material: `RID`, shader: `RID`)

Sets a shader material's shader.

`void (No return value.)` **material_set_use_debanding**(enable: `bool`)
...
`void (No return value.)` **mesh_add_surface**(mesh: `RID`, surface: `Dictionary`)
...
`void (No return value.)` **mesh_add_surface_from_arrays**(mesh: `RID`, primitive: `PrimitiveType`, arrays: `Array`, blend_shapes: `Array` = \[\], lods: `Dictionary` = {}, compress_format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\] = 0)
...
`void (No return value.)` **mesh_clear**(mesh: `RID`)
...
`void (No return value.)` **mesh_set_blend_shape_mode**(mesh: `RID`, mode: `BlendShapeMode`)

Sets a mesh's blend shape mode.

`void (No return value.)` **mesh_set_custom_aabb**(mesh: `RID`, aabb: `AABB`)

Sets a mesh's custom aabb.

`void (No return value.)` **mesh_set_shadow_mesh**(mesh: `RID`, shadow_mesh: `RID`)
...
`int` **mesh_surface_get_format_attribute_stride**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`) `const`

Returns the stride of the attribute buffer for a mesh with given `format`.

`int` **mesh_surface_get_format_index_stride**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`) `const`

Returns the stride of the index buffer for a mesh with the given `format`.

`int` **mesh_surface_get_format_normal_tangent_stride**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`) `const`
...
`int` **mesh_surface_get_format_offset**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`, array_index: `int`) `const`

Returns the offset of a given attribute by `array_index` in the start of its respective buffer.

`int` **mesh_surface_get_format_skin_stride**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`) `const`

Returns the stride of the skin buffer for a mesh with given `format`.

`int` **mesh_surface_get_format_vertex_stride**(format: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\], vertex_count: `int`) `const`
...
`void (No return value.)` **mesh_surface_remove**(mesh: `RID`, surface: `int`)

Removes the surface at the given index from the Mesh, shifting surfaces with higher index down by one.

`void (No return value.)` **mesh_surface_set_material**(mesh: `RID`, surface: `int`, material: `RID`)

Sets a mesh's surface's material.

`void (No return value.)` **mesh_surface_update_attribute_region**(mesh: `RID`, surface: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **mesh_surface_update_index_region**(mesh: `RID`, surface: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **mesh_surface_update_skin_region**(mesh: `RID`, surface: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **mesh_surface_update_vertex_region**(mesh: `RID`, surface: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **multimesh_allocate_data**(multimesh: `RID`, instances: `int`, transform_format: `MultimeshTransformFormat`, color_format: `bool` = false, custom_data_format: `bool` = false, use_indirect: `bool` = false)
...
`void (No return value.)` **multimesh_instance_reset_physics_interpolation**(multimesh: `RID`, index: `int`)

Prevents physics interpolation for the specified instance during the current physics tick.

This is useful when moving an instance to a new location, to give an instantaneous change rather than interpolation from the previous location.

`void (No return value.)` **multimesh_instance_set_color**(multimesh: `RID`, index: `int`, color: `Color`)

Sets the color by which this instance will be modulated. Equivalent to `MultiMesh.set_instance_color()`.

`void (No return value.)` **multimesh_instance_set_custom_data**(multimesh: `RID`, index: `int`, custom_data: `Color`)
...
`void (No return value.)` **multimesh_instance_set_transform**(multimesh: `RID`, index: `int`, transform: `Transform3D`)

Sets the `Transform3D` for this instance. Equivalent to `MultiMesh.set_instance_transform()`.

`void (No return value.)` **multimesh_instance_set_transform_2d**(multimesh: `RID`, index: `int`, transform: `Transform2D`)
...
`void (No return value.)` **multimesh_instances_reset_physics_interpolation**(multimesh: `RID`)
...
`void (No return value.)` **multimesh_set_buffer**(multimesh: `RID`, buffer: `PackedFloat32Array`)
...
`void (No return value.)` **multimesh_set_buffer_interpolated**(multimesh: `RID`, buffer: `PackedFloat32Array`, buffer_previous: `PackedFloat32Array`)

Alternative version of `multimesh_set_buffer()` for use with physics interpolation.

Takes both an array of current data and an array of data for the previous physics tick.

`void (No return value.)` **multimesh_set_custom_aabb**(multimesh: `RID`, aabb: `AABB`)

Sets the custom AABB for this MultiMesh resource.

`void (No return value.)` **multimesh_set_mesh**(multimesh: `RID`, mesh: `RID`)

Sets the mesh to be drawn by the multimesh. Equivalent to `MultiMesh.mesh`.

`void (No return value.)` **multimesh_set_physics_interpolated**(multimesh: `RID`, interpolated: `bool`)

Turns on and off physics interpolation for this MultiMesh resource.

`void (No return value.)` **multimesh_set_physics_interpolation_quality**(multimesh: `RID`, quality: `MultimeshPhysicsInterpolationQuality`)
...
`void (No return value.)` **multimesh_set_visible_instances**(multimesh: `RID`, visible: `int`)
...
`void (No return value.)` **occluder_set_mesh**(occluder: `RID`, vertices: `PackedVector3Array`, indices: `PackedInt32Array`)
...
`void (No return value.)` **particles_collision_height_field_update**(particles_collision: `RID`)
...
`void (No return value.)` **particles_collision_set_attractor_attenuation**(particles_collision: `RID`, curve: `float`)
...
`void (No return value.)` **particles_collision_set_attractor_directionality**(particles_collision: `RID`, amount: `float`)
...
`void (No return value.)` **particles_collision_set_attractor_strength**(particles_collision: `RID`, strength: `float`)
...
`void (No return value.)` **particles_collision_set_box_extents**(particles_collision: `RID`, extents: `Vector3`)
...
`void (No return value.)` **particles_collision_set_collision_type**(particles_collision: `RID`, type: `ParticlesCollisionType`)
...
`void (No return value.)` **particles_collision_set_cull_mask**(particles_collision: `RID`, mask: `int`)
...
`void (No return value.)` **particles_collision_set_field_texture**(particles_collision: `RID`, texture: `RID`)
...
`void (No return value.)` **particles_collision_set_height_field_mask**(particles_collision: `RID`, mask: `int`)
...
`void (No return value.)` **particles_collision_set_height_field_resolution**(particles_collision: `RID`, resolution: `ParticlesCollisionHeightfieldResolution`)
...
`void (No return value.)` **particles_collision_set_sphere_radius**(particles_collision: `RID`, radius: `float`)
...
`void (No return value.)` **particles_emit**(particles: `RID`, transform: `Transform3D`, velocity: `Vector3`, color: `Color`, custom: `Color`, emit_flags: `int`)
...
`void (No return value.)` **particles_request_process**(particles: `RID`)
...
`void (No return value.)` **particles_request_process_time**(particles: `RID`, process_time: `float`, process_time_residual: `float` = 0.0)
...
`void (No return value.)` **particles_set_draw_passes**(particles: `RID`, count: `int`)

Sets the number of draw passes to use. Equivalent to `GPUParticles3D.draw_passes`.

`void (No return value.)` **particles_set_emission_transform**(particles: `RID`, transform: `Transform3D`)

Sets the `Transform3D` that will be used by the particles when they first emit.

`void (No return value.)` **particles_set_emitter_velocity**(particles: `RID`, velocity: `Vector3`)

Sets the velocity of a particle node, that will be used by `ParticleProcessMaterial.inherit_velocity_ratio`.

`void (No return value.)` **particles_set_emitting**(particles: `RID`, emitting: `bool`)
...
`void (No return value.)` **particles_set_process_material**(particles: `RID`, material: `RID`)

Sets the material for processing the particles.

**Note:** This is not the material used to draw the materials. Equivalent to `GPUParticles3D.process_material`.

`void (No return value.)` **particles_set_randomness_ratio**(particles: `RID`, ratio: `float`)
...
`void (No return value.)` **particles_set_speed_scale**(particles: `RID`, scale: `float`)

Sets the speed scale of the particle system. Equivalent to `GPUParticles3D.speed_scale`.

`void (No return value.)` **particles_set_subemitter**(particles: `RID`, subemitter_particles: `RID`)

Sets the subemitter particles for the particle system. Equivalent to `GPUParticles3D.sub_emitter`.

`void (No return value.)` **particles_set_trail_bind_poses**(particles: `RID`, bind_poses: `Array`\[`Transform3D`\])
...
`void (No return value.)` **particles_set_trails**(particles: `RID`, enable: `bool`, length_sec: `float`)
...
`void (No return value.)` **particles_set_transform_align**(particles: `RID`, align: `ParticlesTransformAlign`)

Sets the transform alignment for the particle system. Equivalent to `GPUParticles3D.transform_align`.

`void (No return value.)` **particles_set_transform_align_axis**(particles: `RID`, rotation_axis: `ParticlesTransformAlignAxis`)

Sets which axis to use for transform alignment.

`void (No return value.)` **particles_set_transform_align_channel_filter**(particles: `RID`, channel_filter: `ParticlesTransformAlignCustomSrc`)

When using Z-Billboarding, which CUSTOM channel to read from.

`void (No return value.)` **particles_set_use_local_coordinates**(particles: `RID`, enable: `bool`)

If `true`, particles use local coordinates. If `false` they use global coordinates. Equivalent to `GPUParticles3D.local_coords`.

`void (No return value.)` **positional_soft_shadow_filter_set_quality**(quality: `ShadowQuality`)
...
`void (No return value.)` **reflection_probe_set_ambient_color**(probe: `RID`, color: `Color`)

Sets the reflection probe's custom ambient light color. Equivalent to `ReflectionProbe.ambient_color`.

`void (No return value.)` **reflection_probe_set_ambient_energy**(probe: `RID`, energy: `float`)

Sets the reflection probe's custom ambient light energy. Equivalent to `ReflectionProbe.ambient_color_energy`.

`void (No return value.)` **reflection_probe_set_ambient_mode**(probe: `RID`, mode: `ReflectionProbeAmbientMode`)

Sets the reflection probe's ambient light mode. Equivalent to `ReflectionProbe.ambient_mode`.

`void (No return value.)` **reflection_probe_set_as_interior**(probe: `RID`, enable: `bool`)

If `true`, reflections will ignore sky contribution. Equivalent to `ReflectionProbe.interior`.

`void (No return value.)` **reflection_probe_set_blend_distance**(probe: `RID`, blend_distance: `float`)

Sets the distance in meters over which a probe blends into the scene.

`void (No return value.)` **reflection_probe_set_cull_mask**(probe: `RID`, layers: `int`)
...
`void (No return value.)` **reflection_probe_set_enable_box_projection**(probe: `RID`, enable: `bool`)
...
`void (No return value.)` **reflection_probe_set_enable_shadows**(probe: `RID`, enable: `bool`)
...
`void (No return value.)` **reflection_probe_set_intensity**(probe: `RID`, intensity: `float`)
...
`void (No return value.)` **reflection_probe_set_max_distance**(probe: `RID`, distance: `float`)
...
`void (No return value.)` **reflection_probe_set_mesh_lod_threshold**(probe: `RID`, pixels: `float`)
...
`void (No return value.)` **reflection_probe_set_origin_offset**(probe: `RID`, offset: `Vector3`)
...
`void (No return value.)` **reflection_probe_set_reflection_mask**(probe: `RID`, layers: `int`)
...
`void (No return value.)` **reflection_probe_set_resolution**(probe: `RID`, resolution: `int`)

**Deprecated:** This method has not done anything since Godot 3.

Deprecated. This method does nothing.

`void (No return value.)` **reflection_probe_set_size**(probe: `RID`, size: `Vector3`)

Sets the size of the area that the reflection probe will capture. Equivalent to `ReflectionProbe.size`.

`void (No return value.)` **reflection_probe_set_update_mode**(probe: `RID`, mode: `ReflectionProbeUpdateMode`)

Sets how often the reflection probe updates. Can either be once or every frame.

`void (No return value.)` **request_frame_drawn_callback**(callable: `Callable`)
...
`void (No return value.)` **scenario_set_camera_attributes**(scenario: `RID`, effects: `RID`)

Sets the camera attributes (`effects`) that will be used with this scenario. See also `CameraAttributes`.

`void (No return value.)` **scenario_set_compositor**(scenario: `RID`, compositor: `RID`)

Sets the compositor (`compositor`) that will be used with this scenario. See also `Compositor`.

`void (No return value.)` **scenario_set_environment**(scenario: `RID`, environment: `RID`)

Sets the environment that will be used with this scenario. See also `Environment`.

`void (No return value.)` **scenario_set_fallback_environment**(scenario: `RID`, environment: `RID`)

Sets the fallback environment to be used by this scenario. The fallback environment is used if no environment is set. Internally, this is used by the editor to provide a default environment.

`void (No return value.)` **screen_space_roughness_limiter_set_active**(enable: `bool`, amount: `float`, limit: `float`)
...
`void (No return value.)` **set_boot_image**(image: `Image`, color: `Color`, scale: `bool`, use_filter: `bool` = true)
...
`void (No return value.)` **set_boot_image_with_stretch**(image: `Image`, color: `Color`, stretch_mode: `SplashStretchMode`, use_filter: `bool` = true)
...
`void (No return value.)` **set_debug_generate_wireframes**(generate: `bool`)
...
`void (No return value.)` **set_default_clear_color**(color: `Color`)
...
`void (No return value.)` **shader_set_code**(shader: `RID`, code: `String`)

Sets the shader's source code (which triggers recompilation after being changed).

`void (No return value.)` **shader_set_default_texture_parameter**(shader: `RID`, name: `StringName`, texture: `RID`, index: `int` = 0)

Sets a shader's default texture. Overwrites the texture given by name.

**Note:** If the sampler array is used use `index` to access the specified texture.

`void (No return value.)` **shader_set_path_hint**(shader: `RID`, path: `String`)

Sets the path hint for the specified shader. This should generally match the `Shader` resource's `Resource.resource_path`.

`void (No return value.)` **skeleton_allocate_data**(skeleton: `RID`, bones: `int`, is_2d_skeleton: `bool` = false)
...
`void (No return value.)` **skeleton_bone_set_transform**(skeleton: `RID`, bone: `int`, transform: `Transform3D`)

Sets the `Transform3D` for a specific bone of this skeleton.

`void (No return value.)` **skeleton_bone_set_transform_2d**(skeleton: `RID`, bone: `int`, transform: `Transform2D`)
...
`void (No return value.)` **skeleton_set_base_transform_2d**(skeleton: `RID`, base_transform: `Transform2D`)
...
`void (No return value.)` **sky_set_material**(sky: `RID`, material: `RID`)

Sets the material that the sky uses to render the background, ambient and reflection maps.

`void (No return value.)` **sky_set_mode**(sky: `RID`, mode: `SkyMode`)

Sets the process `mode` of the sky specified by the `sky` RID. Equivalent to `Sky.process_mode`.

`void (No return value.)` **sky_set_radiance_size**(sky: `RID`, radiance_size: `int`)
...
`void (No return value.)` **sub_surface_scattering_set_quality**(quality: `SubSurfaceScatteringQuality`)
...
`void (No return value.)` **sub_surface_scattering_set_scale**(scale: `float`, depth_scale: `float`)
...
`void (No return value.)` **texture_2d_update**(texture: `RID`, image: `Image`, layer: `int`)
...
`void (No return value.)` **texture_3d_update**(texture: `RID`, data: `Array`\[`Image`\])
...
`void (No return value.)` **texture_drawable_blit_rect**(textures: `Array`\[`RID`\], rect: `Rect2i`, material: `RID`, modulate: `Color`, source_textures: `Array`\[`RID`\], to_mipmap: `int` = 0)
...
`void (No return value.)` **texture_drawable_generate_mipmaps**(texture: `RID`)
...
`void (No return value.)` **texture_proxy_update**(texture: `RID`, proxy_to: `RID`)
...
`void (No return value.)` **texture_replace**(texture: `RID`, by_texture: `RID`)

Replaces `texture`'s texture data by the texture specified by the `by_texture` RID, without changing `texture`'s RID.

`void (No return value.)` **texture_set_force_redraw_if_visible**(texture: `RID`, enable: `bool`)
...
`void (No return value.)` **texture_set_path**(texture: `RID`, path: `String`)
...
`void (No return value.)` **texture_set_size_override**(texture: `RID`, width: `int`, height: `int`)
...
`void (No return value.)` **viewport_attach_camera**(viewport: `RID`, camera: `RID`)

Sets a viewport's camera.

`void (No return value.)` **viewport_attach_canvas**(viewport: `RID`, canvas: `RID`)

Sets a viewport's canvas.

`void (No return value.)` **viewport_attach_to_screen**(viewport: `RID`, rect: `Rect2` = Rect2(0, 0, 0, 0), screen: `int` = 0)
...
`void (No return value.)` **viewport_remove_canvas**(viewport: `RID`, canvas: `RID`)

Detaches a viewport from a canvas.

`void (No return value.)` **viewport_set_active**(viewport: `RID`, active: `bool`)

If `true`, sets the viewport active, else sets it inactive.

`void (No return value.)` **viewport_set_anisotropic_filtering_level**(viewport: `RID`, anisotropic_filtering_level: `ViewportAnisotropicFiltering`)
...
`void (No return value.)` **viewport_set_canvas_cull_mask**(viewport: `RID`, canvas_cull_mask: `int`)

Sets the rendering mask associated with this `Viewport`. Only `CanvasItem` nodes with a matching rendering visibility layer will be rendered by this `Viewport`.

`void (No return value.)` **viewport_set_canvas_stacking**(viewport: `RID`, canvas: `RID`, layer: `int`, sublayer: `int`)
...
`void (No return value.)` **viewport_set_canvas_transform**(viewport: `RID`, canvas: `RID`, offset: `Transform2D`)

Sets the transformation of a viewport's canvas.

`void (No return value.)` **viewport_set_clear_mode**(viewport: `RID`, clear_mode: `ViewportClearMode`)

Sets the clear mode of a viewport.

`void (No return value.)` **viewport_set_debug_draw**(viewport: `RID`, draw: `ViewportDebugDraw`)

Sets the debug draw mode of a viewport.

`void (No return value.)` **viewport_set_default_canvas_item_texture_filter**(viewport: `RID`, filter: `CanvasItemTextureFilter`)

Sets the default texture filtering mode for the specified `viewport` RID.

`void (No return value.)` **viewport_set_default_canvas_item_texture_repeat**(viewport: `RID`, repeat: `CanvasItemTextureRepeat`)

Sets the default texture repeat mode for the specified `viewport` RID.

`void (No return value.)` **viewport_set_disable_2d**(viewport: `RID`, disable: `bool`)

If `true`, the viewport's canvas (i.e. 2D and GUI elements) is not rendered.

`void (No return value.)` **viewport_set_disable_3d**(viewport: `RID`, disable: `bool`)

If `true`, the viewport's 3D elements are not rendered.

`void (No return value.)` **viewport_set_environment_mode**(viewport: `RID`, mode: `ViewportEnvironmentMode`)
...
`void (No return value.)` **viewport_set_fsr_sharpness**(viewport: `RID`, sharpness: `float`)

Determines how sharp the upscaled image will be when using the FSR upscaling mode. Sharpness halves with every whole number. Values go from 0.0 (sharpest) to 2.0. Values above 2.0 won't make a visible difference.

`void (No return value.)` **viewport_set_global_canvas_transform**(viewport: `RID`, transform: `Transform2D`)

Sets the viewport's global transformation matrix.

`void (No return value.)` **viewport_set_measure_render_time**(viewport: `RID`, enable: `bool`)
...
`void (No return value.)` **viewport_set_msaa_2d**(viewport: `RID`, msaa: `ViewportMSAA`)
...
`void (No return value.)` **viewport_set_msaa_3d**(viewport: `RID`, msaa: `ViewportMSAA`)
...
`void (No return value.)` **viewport_set_occlusion_culling_build_quality**(quality: `ViewportOcclusionCullingBuildQuality`)
...
`void (No return value.)` **viewport_set_occlusion_rays_per_thread**(rays_per_thread: `int`)
...
`void (No return value.)` **viewport_set_parent_viewport**(viewport: `RID`, parent_viewport: `RID`)

Sets the viewport's parent to the viewport specified by the `parent_viewport` RID.

`void (No return value.)` **viewport_set_positional_shadow_atlas_quadrant_subdivision**(viewport: `RID`, quadrant: `int`, subdivision: `int`)
...
`void (No return value.)` **viewport_set_positional_shadow_atlas_size**(viewport: `RID`, size: `int`, use_16_bits: `bool` = false)
...
`void (No return value.)` **viewport_set_render_direct_to_screen**(viewport: `RID`, enabled: `bool`)
...
`void (No return value.)` **viewport_set_scaling_3d_mode**(viewport: `RID`, scaling_3d_mode: `ViewportScaling3DMode`)

Sets the 3D resolution scaling mode. Bilinear scaling renders at different resolution to either undersample or supersample the viewport. FidelityFX Super Resolution 1.0, abbreviated to FSR, is an upscaling technology that produces high quality images at fast framerates by using a spatially aware upscaling algorithm. FSR is slightly more expensive than bilinear, but it produces significantly higher image quality. FSR should be used where possible.

`void (No return value.)` **viewport_set_scaling_3d_scale**(viewport: `RID`, scale: `float`)
...
`void (No return value.)` **viewport_set_scenario**(viewport: `RID`, scenario: `RID`)

Sets a viewport's scenario. The scenario contains information about environment information, reflection atlas, etc.

`void (No return value.)` **viewport_set_screen_space_aa**(viewport: `RID`, mode: `ViewportScreenSpaceAA`)
...
`void (No return value.)` **viewport_set_sdf_oversize_and_scale**(viewport: `RID`, oversize: `ViewportSDFOversize`, scale: `ViewportSDFScale`)
...
`void (No return value.)` **viewport_set_size**(viewport: `RID`, width: `int`, height: `int`, view_count: `int` = 1)
...
`void (No return value.)` **viewport_set_snap_2d_transforms_to_pixel**(viewport: `RID`, enabled: `bool`)
...
`void (No return value.)` **viewport_set_snap_2d_vertices_to_pixel**(viewport: `RID`, enabled: `bool`)
...
`void (No return value.)` **viewport_set_texture_mipmap_bias**(viewport: `RID`, mipmap_bias: `float`)
...
`void (No return value.)` **viewport_set_transparent_background**(viewport: `RID`, enabled: `bool`)

If `true`, the viewport renders its background as transparent.

`void (No return value.)` **viewport_set_update_mode**(viewport: `RID`, update_mode: `ViewportUpdateMode`)

Sets when the viewport should be updated.

`void (No return value.)` **viewport_set_use_debanding**(viewport: `RID`, enable: `bool`)

Equivalent to `Viewport.use_debanding`. See also `ProjectSettings.rendering/anti_aliasing/quality/use_debanding`.

`void (No return value.)` **viewport_set_use_hdr_2d**(viewport: `RID`, enabled: `bool`)
...
`void (No return value.)` **viewport_set_use_occlusion_culling**(viewport: `RID`, enable: `bool`)
...
`void (No return value.)` **viewport_set_use_taa**(viewport: `RID`, enable: `bool`)
...
`void (No return value.)` **viewport_set_use_xr**(viewport: `RID`, use_xr: `bool`)

If `true`, the viewport uses augmented or virtual reality technologies. See `XRInterface`.

`void (No return value.)` **viewport_set_vrs_mode**(viewport: `RID`, mode: `ViewportVRSMode`)
...
`void (No return value.)` **viewport_set_vrs_texture**(viewport: `RID`, texture: `RID`)

The texture to use when the VRS mode is set to `VIEWPORT_VRS_TEXTURE`. Equivalent to `ProjectSettings.rendering/vrs/texture`.

`void (No return value.)` **viewport_set_vrs_update_mode**(viewport: `RID`, mode: `ViewportVRSUpdateMode`)
...
`void (No return value.)` **visibility_notifier_set_aabb**(notifier: `RID`, aabb: `AABB`)

Sets the AABB of the specified visibility notifier.

`void (No return value.)` **visibility_notifier_set_callbacks**(notifier: `RID`, enter_callable: `Callable`, exit_callable: `Callable`)

Sets the methods to be called when the notifier enters or exits the view.

`void (No return value.)` **voxel_gi_allocate_data**(voxel_gi: `RID`, to_cell_xform: `Transform3D`, aabb: `AABB`, octree_size: `Vector3i`, octree_cells: `PackedByteArray`, data_cells: `PackedByteArray`, distance_field: `PackedByteArray`, level_counts: `PackedInt32Array`)
...
`void (No return value.)` **voxel_gi_set_baked_exposure_normalization**(voxel_gi: `RID`, baked_exposure: `float`)
...
`void (No return value.)` **voxel_gi_set_bias**(voxel_gi: `RID`, bias: `float`)

Sets the `VoxelGIData.bias` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_dynamic_range**(voxel_gi: `RID`, range: `float`)

Sets the `VoxelGIData.dynamic_range` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_energy**(voxel_gi: `RID`, energy: `float`)

Sets the `VoxelGIData.energy` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_interior**(voxel_gi: `RID`, enable: `bool`)

Sets the `VoxelGIData.interior` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_normal_bias**(voxel_gi: `RID`, bias: `float`)

Sets the `VoxelGIData.normal_bias` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_propagation**(voxel_gi: `RID`, amount: `float`)

Sets the `VoxelGIData.propagation` value to use on the specified `voxel_gi`'s `RID`.

`void (No return value.)` **voxel_gi_set_quality**(quality: `VoxelGIQuality`)
...
`void (No return value.)` **voxel_gi_set_use_two_bounces**(voxel_gi: `RID`, enable: `bool`)
