...
- `void (No return value.)` **set_blend_shape_mode**(value: `BlendShapeMode`)
...
- `void (No return value.)` **set_shadow_mesh**(value: `ArrayMesh`)
...
`void (No return value.)` **add_blend_shape**(name: `StringName`)

Adds name for a blend shape that will be added with `add_surface_from_arrays()`. Must be called before surface is added.

`void (No return value.)` **add_surface_from_arrays**(primitive: `PrimitiveType`, arrays: `Array`, blend_shapes: `Array`\[`Array`\] = \[\], lods: `Dictionary` = {}, flags: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\] = 0)
...
`void (No return value.)` **clear_blend_shapes**()
...
`void (No return value.)` **regen_normal_maps**()

Regenerates tangents for each of the **ArrayMesh**'s surfaces.

`void (No return value.)` **set_blend_shape_name**(index: `int`, name: `StringName`)
...
`BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ArrayFormat`\] **surface_get_format**(surf_idx: `int`) `const`
...
`void (No return value.)` **surface_remove**(surf_idx: `int`)

Removes the surface at the given index from the Mesh, shifting surfaces with higher index down by one.

`void (No return value.)` **surface_set_name**(surf_idx: `int`, name: `String`)

Sets a name for a given surface.

`void (No return value.)` **surface_update_attribute_region**(surf_idx: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **surface_update_skin_region**(surf_idx: `int`, offset: `int`, data: `PackedByteArray`)
...
`void (No return value.)` **surface_update_vertex_region**(surf_idx: `int`, offset: `int`, data: `PackedByteArray`)
