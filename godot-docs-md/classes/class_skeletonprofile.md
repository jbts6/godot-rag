...
- `void (No return value.)` **set_bone_size**(value: `int`)
- `int` **get_bone_size**()
...
- `void (No return value.)` **set_group_size**(value: `int`)
- `int` **get_group_size**()
...
- `void (No return value.)` **set_root_bone**(value: `StringName`)
...
- `void (No return value.)` **set_scale_base_bone**(value: `StringName`)
...
`void (No return value.)` **set_bone_name**(bone_idx: `int`, bone_name: `StringName`)

Sets the name of the bone at `bone_idx` that will be the key name in the `BoneMap`.

In the retargeting process, the setting bone name is the bone name of the target skeleton.

`void (No return value.)` **set_bone_parent**(bone_idx: `int`, bone_parent: `StringName`)
...
`void (No return value.)` **set_bone_tail**(bone_idx: `int`, bone_tail: `StringName`)

Sets the bone with name `bone_tail` as the tail of the bone at `bone_idx`.

`void (No return value.)` **set_group**(bone_idx: `int`, group: `StringName`)

Sets the group of the bone at `bone_idx`.

`void (No return value.)` **set_group_name**(group_idx: `int`, group_name: `StringName`)

Sets the name of the group at `group_idx` that will be the drawing group in the `BoneMap` editor.

`void (No return value.)` **set_handle_offset**(bone_idx: `int`, handle_offset: `Vector2`)

Sets the offset of the bone at `bone_idx` that will be the button position in the `BoneMap` editor.

This is the offset with origin at the top left corner of the square.

`void (No return value.)` **set_reference_pose**(bone_idx: `int`, bone_name: `Transform3D`)

Sets the reference pose transform for bone `bone_idx`.

`void (No return value.)` **set_required**(bone_idx: `int`, required: `bool`)

Sets the required status for bone `bone_idx` to `required`.

`void (No return value.)` **set_tail_direction**(bone_idx: `int`, tail_direction: `TailDirection`)

Sets the tail direction of the bone at `bone_idx`.

**Note:** This only specifies the method of calculation. The actual coordinates required should be stored in an external skeleton, so the calculation itself needs to be done externally.

`void (No return value.)` **set_texture**(group_idx: `int`, texture: `Texture2D`)
