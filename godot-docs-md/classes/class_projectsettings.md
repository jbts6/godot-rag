...
The size of the shadow atlas used for `OmniLight3D` and `SpotLight3D` nodes. See the [documentation](../tutorials/3d/lights_and_shadows.html#shadow-atlas) for more information.
...
`void (No return value.)` **add_property_info**(hint: `Dictionary`)
...
`void (No return value.)` **clear**(name: `String`)
...
`void (No return value.)` **set_as_basic**(name: `String`, basic: `bool`)

Defines if the specified setting is considered basic or advanced. Basic settings will always be shown in the project settings. Advanced settings will only be shown if the user enables the "Advanced Settings" option.

`void (No return value.)` **set_as_internal**(name: `String`, internal: `bool`)

Defines if the specified setting is considered internal. An internal setting won't show up in the Project Settings dialog. This is mostly useful for addons that need to store their own internal settings without exposing them directly to the user.

`void (No return value.)` **set_initial_value**(name: `String`, value: `Variant`)
...
`void (No return value.)` **set_order**(name: `String`, position: `int`)

Sets the order of a configuration value (influences when saved to the config file).

`void (No return value.)` **set_restart_if_changed**(name: `String`, restart: `bool`)
