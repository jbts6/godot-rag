...
`MultiplayerPeer` **\_get_multiplayer_peer**() `virtual (This method should typically be overridden by the user to have any effect.)`

Called when the `MultiplayerAPI.multiplayer_peer` is retrieved.

`PackedInt32Array` **\_get_peer_ids**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

Callback for `MultiplayerAPI.get_peers()`.

`int` **\_get_remote_sender_id**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

Callback for `MultiplayerAPI.get_remote_sender_id()`.

`int` **\_get_unique_id**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

Callback for `MultiplayerAPI.get_unique_id()`.

`Error` **\_object_configuration_add**(object: `Object`, configuration: `Variant`) `virtual (This method should typically be overridden by the user to have any effect.)`

Callback for `MultiplayerAPI.object_configuration_add()`.

`Error` **\_object_configuration_remove**(object: `Object`, configuration: `Variant`) `virtual (This method should typically be overridden by the user to have any effect.)`

Callback for `MultiplayerAPI.object_configuration_remove()`.

`Error` **\_poll**() `virtual (This method should typically be overridden by the user to have any effect.)`

Callback for `MultiplayerAPI.poll()`.

`Error` **\_rpc**(peer: `int`, object: `Object`, method: `StringName`, args: `Array`) `virtual (This method should typically be overridden by the user to have any effect.)`

Callback for `MultiplayerAPI.rpc()`.

`void (No return value.)` **\_set_multiplayer_peer**(multiplayer_peer: `MultiplayerPeer`) `virtual (This method should typically be overridden by the user to have any effect.)`
