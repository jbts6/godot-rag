...
- `void (No return value.)` **set_bake_resolution**(value: `int`)
...
- `void (No return value.)` **set_max_domain**(value: `float`)
...
- `void (No return value.)` **set_max_value**(value: `float`)
- `float` **get_max_value**()
...
- `void (No return value.)` **set_min_domain**(value: `float`)
...
- `void (No return value.)` **set_min_value**(value: `float`)
- `float` **get_min_value**()
...
`void (No return value.)` **bake**()

Recomputes the baked cache of points for the curve.

`void (No return value.)` **clean_dupes**()

Removes duplicate points, i.e. points that are less than 0.00001 units (engine epsilon value) away from their neighbor on the curve.

`void (No return value.)` **clear_points**()
...
`void (No return value.)` **remove_point**(index: `int`)
...
`void (No return value.)` **set_point_left_mode**(index: `int`, mode: `TangentMode`)

Sets the left `TangentMode` for the point at `index` to `mode`.

`void (No return value.)` **set_point_left_tangent**(index: `int`, tangent: `float`)
...
`void (No return value.)` **set_point_right_mode**(index: `int`, mode: `TangentMode`)

Sets the right `TangentMode` for the point at `index` to `mode`.

`void (No return value.)` **set_point_right_tangent**(index: `int`, tangent: `float`)

Sets the right tangent angle for the point at `index` to `tangent`.

`void (No return value.)` **set_point_value**(index: `int`, y: `float`)
