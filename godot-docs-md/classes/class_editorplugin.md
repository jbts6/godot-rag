...
`void (No return value.)` **\_apply_changes**() `virtual (This method should typically be overridden by the user to have any effect.)`

This method is called when the editor is about to save the project, switch to another tab, etc. It asks the plugin to apply any pending state changes to ensure consistency.

This is used, for example, in shader editors to let the plugin know that it must apply the shader code being written by the user to the object.

`bool` **\_build**() `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_clear**() `virtual (This method should typically be overridden by the user to have any effect.)`

Clear all the state and reset the object being edited to zero. This ensures your plugin does not keep editing a currently existing node, or a node from the wrong scene.

`void (No return value.)` **\_disable_plugin**() `virtual (This method should typically be overridden by the user to have any effect.)`

Called by the engine when the user disables the **EditorPlugin** in the Plugin tab of the project settings window.

`void (No return value.)` **\_edit**(object: `Object`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_enable_plugin**() `virtual (This method should typically be overridden by the user to have any effect.)`

Called by the engine when the user enables the **EditorPlugin** in the Plugin tab of the project settings window.

`void (No return value.)` **\_forward_3d_draw_over_viewport**(viewport_control: `Control`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_forward_3d_force_draw_over_viewport**(viewport_control: `Control`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`int` **\_forward_3d_gui_input**(viewport_camera: `Camera3D`, event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_forward_canvas_draw_over_viewport**(viewport_control: `Control`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_forward_canvas_force_draw_over_viewport**(viewport_control: `Control`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`bool` **\_forward_canvas_gui_input**(event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`PackedStringArray` **\_get_breakpoints**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

This is for editors that edit script-based objects. You can return a list of breakpoints in the format (`script:line`), for example: `res://path_to_script.gd:25`.

`Texture2D` **\_get_plugin_icon**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`String` **\_get_plugin_name**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`

Override this method in your plugin to provide the name of the plugin when displayed in the Godot editor.

For main screen plugins, this appears at the top of the screen, to the right of the "2D", "3D", "Script", "Game", and "Asset Store" buttons.

`Dictionary` **\_get_state**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`String` **\_get_unsaved_status**(for_scene: `String`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`void (No return value.)` **\_get_window_layout**(configuration: `ConfigFile`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`bool` **\_handles**(object: `Object`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`bool` **\_has_main_screen**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`void (No return value.)` **\_make_visible**(visible: `bool`) `virtual (This method should typically be overridden by the user to have any effect.)`

This function will be called when the editor is requested to become visible. It is used for plugins that edit a specific object type.

Remember that you have to manage the visibility of all your editor controls manually.

`PackedStringArray` **\_run_scene**(scene: `String`, args: `PackedStringArray`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`void (No return value.)` **\_save_external_data**() `virtual (This method should typically be overridden by the user to have any effect.)`

This method is called after the editor saves the project or when it's closed. It asks the plugin to save edited external scenes/resources.

`void (No return value.)` **\_set_state**(state: `Dictionary`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_set_window_layout**(configuration: `ConfigFile`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **add_autoload_singleton**(name: `String`, path: `String`)

Adds a script at `path` to the Autoload list as `name`.

`void (No return value.)` **add_context_menu_plugin**(slot: `ContextMenuSlot`, plugin: `EditorContextMenuPlugin`)
...
`void (No return value.)` **add_control_to_container**(container: `CustomControlContainer`, control: `Control`)
...
`void (No return value.)` **add_control_to_dock**(slot: `DockSlot`, control: `Control`, shortcut: `Shortcut` = null)
...
`void (No return value.)` **add_custom_type**(type: `String`, base: `String`, script: `Script`, icon: `Texture2D`)
...
`void (No return value.)` **add_debugger_plugin**(script: `EditorDebuggerPlugin`)

Adds a `Script` as debugger plugin to the Debugger. The script must extend `EditorDebuggerPlugin`.

`void (No return value.)` **add_dock**(dock: `EditorDock`)

Adds a new dock.

When your plugin is deactivated, make sure to remove your custom dock with `remove_dock()` and free it with `Node.queue_free()`.

`void (No return value.)` **add_export_platform**(platform: `EditorExportPlatform`)

Registers a new `EditorExportPlatform`. Export platforms provides functionality of exporting to the specific platform.

`void (No return value.)` **add_export_plugin**(plugin: `EditorExportPlugin`)

Registers a new `EditorExportPlugin`. Export plugins are used to perform tasks when the project is being exported.

See `add_inspector_plugin()` for an example of how to register a plugin.

`void (No return value.)` **add_import_plugin**(importer: `EditorImportPlugin`, first_priority: `bool` = false)
...
`void (No return value.)` **add_inspector_plugin**(plugin: `EditorInspectorPlugin`)
...
`void (No return value.)` **add_node_3d_gizmo_plugin**(plugin: `EditorNode3DGizmoPlugin`)

Registers a new `EditorNode3DGizmoPlugin`. Gizmo plugins are used to add custom gizmos to the 3D preview viewport for a `Node3D`.

See `add_inspector_plugin()` for an example of how to register a plugin.

`void (No return value.)` **add_resource_conversion_plugin**(plugin: `EditorResourceConversionPlugin`)
...
`void (No return value.)` **add_scene_format_importer_plugin**(scene_format_importer: `EditorSceneFormatImporter`, first_priority: `bool` = false)

Registers a new `EditorSceneFormatImporter`. Scene importers are used to import custom 3D asset formats as scenes.

If `first_priority` is `true`, the new import plugin is inserted first in the list and takes precedence over pre-existing plugins.

`void (No return value.)` **add_scene_post_import_plugin**(scene_import_plugin: `EditorScenePostImportPlugin`, first_priority: `bool` = false)
...
`void (No return value.)` **add_tool_menu_item**(name: `String`, callable: `Callable`)

Adds a custom menu item to **Project \> Tools** named `name`. When clicked, the provided `callable` will be called.

`void (No return value.)` **add_tool_submenu_item**(name: `String`, submenu: `PopupMenu`)

Adds a custom `PopupMenu` submenu under **Project \> Tools \>** `name`. Use `remove_tool_menu_item()` on plugin clean up to remove the menu.

`void (No return value.)` **add_translation_parser_plugin**(parser: `EditorTranslationParserPlugin`)

Registers a custom translation parser plugin for extracting translatable strings from custom files.

`void (No return value.)` **add_undo_redo_inspector_hook_callback**(callable: `Callable`)
...
`void (No return value.)` **hide_bottom_panel**()

Minimizes the bottom panel.

`void (No return value.)` **make_bottom_panel_item_visible**(item: `Control`)

Makes a specific item in the bottom panel visible.

`void (No return value.)` **queue_save_layout**()

Queue save the project's editor layout.

`void (No return value.)` **remove_autoload_singleton**(name: `String`)

Removes an Autoload `name` from the list.

`void (No return value.)` **remove_context_menu_plugin**(plugin: `EditorContextMenuPlugin`)

Removes the specified context menu plugin.

`void (No return value.)` **remove_control_from_bottom_panel**(control: `Control`)
...
`void (No return value.)` **remove_control_from_container**(container: `CustomControlContainer`, control: `Control`)

Removes the control from the specified container. You have to manually `Node.queue_free()` the control.

`void (No return value.)` **remove_control_from_docks**(control: `Control`)

**Deprecated:** Use `remove_dock()` instead.

Removes the control from the dock. You have to manually `Node.queue_free()` the control.

`void (No return value.)` **remove_custom_type**(type: `String`)

Removes a custom type added by `add_custom_type()`.

`void (No return value.)` **remove_debugger_plugin**(script: `EditorDebuggerPlugin`)

Removes the debugger plugin with given script from the Debugger.

`void (No return value.)` **remove_dock**(dock: `EditorDock`)

Removes `dock` from the available docks. You should manually call `Node.queue_free()` to free it.

`void (No return value.)` **remove_export_platform**(platform: `EditorExportPlatform`)

Removes an export platform registered by `add_export_platform()`.

`void (No return value.)` **remove_export_plugin**(plugin: `EditorExportPlugin`)

Removes an export plugin registered by `add_export_plugin()`.

`void (No return value.)` **remove_import_plugin**(importer: `EditorImportPlugin`)

Removes an import plugin registered by `add_import_plugin()`.

`void (No return value.)` **remove_inspector_plugin**(plugin: `EditorInspectorPlugin`)

Removes an inspector plugin registered by `add_inspector_plugin()`.

`void (No return value.)` **remove_node_3d_gizmo_plugin**(plugin: `EditorNode3DGizmoPlugin`)

Removes a gizmo plugin registered by `add_node_3d_gizmo_plugin()`.

`void (No return value.)` **remove_resource_conversion_plugin**(plugin: `EditorResourceConversionPlugin`)

Removes a resource conversion plugin registered by `add_resource_conversion_plugin()`.

`void (No return value.)` **remove_scene_format_importer_plugin**(scene_format_importer: `EditorSceneFormatImporter`)

Removes a scene format importer registered by `add_scene_format_importer_plugin()`.

`void (No return value.)` **remove_scene_post_import_plugin**(scene_import_plugin: `EditorScenePostImportPlugin`)

Remove the `EditorScenePostImportPlugin`, added with `add_scene_post_import_plugin()`.

`void (No return value.)` **remove_tool_menu_item**(name: `String`)

Removes a menu `name` from **Project \> Tools**.

`void (No return value.)` **remove_translation_parser_plugin**(parser: `EditorTranslationParserPlugin`)

Removes a custom translation parser plugin registered by `add_translation_parser_plugin()`.

`void (No return value.)` **remove_undo_redo_inspector_hook_callback**(callable: `Callable`)

Removes a callback previously added by `add_undo_redo_inspector_hook_callback()`.

`void (No return value.)` **set_dock_tab_icon**(control: `Control`, icon: `Texture2D`)
...
`void (No return value.)` **set_force_draw_over_forwarding_enabled**()
...
`void (No return value.)` **set_input_event_forwarding_always_enabled**()
