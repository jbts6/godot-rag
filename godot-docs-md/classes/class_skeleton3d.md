...
- `void (No return value.)` **set_animate_physical_bones**(value: `bool`)
...
- `void (No return value.)` **set_modifier_callback_mode_process**(value: `ModifierCallbackModeProcess`)
...
- `void (No return value.)` **set_motion_scale**(value: `float`)
...
- `void (No return value.)` **set_show_rest_only**(value: `bool`)
...
`void (No return value.)` **advance**(delta: `float`)
...
`void (No return value.)` **clear_bones_global_pose_override**()
...
`void (No return value.)` **force_update_all_bone_transforms**()

**Deprecated:** This method should only be called internally.

Force updates the bone transforms/poses for all bones in the skeleton.

`void (No return value.)` **force_update_bone_child_transform**(bone_idx: `int`)
...
`void (No return value.)` **localize_rests**()
...
`void (No return value.)` **reset_bone_pose**(bone_idx: `int`)

Sets the bone pose to rest for `bone_idx`.

`void (No return value.)` **reset_bone_poses**()

Sets all bone poses to rests.

`void (No return value.)` **set_bone_enabled**(bone_idx: `int`, enabled: `bool` = true)

Disables the pose for the bone at `bone_idx` if `false`, enables the bone pose if `true`.

`void (No return value.)` **set_bone_global_pose**(bone_idx: `int`, pose: `Transform3D`)
...
`void (No return value.)` **set_bone_global_pose_override**(bone_idx: `int`, pose: `Transform3D`, amount: `float`, persistent: `bool` = false)
...
`void (No return value.)` **set_bone_meta**(bone_idx: `int`, key: `StringName`, value: `Variant`)

Sets the metadata with the given `key` to `value` for the bone at index `bone_idx`.

`void (No return value.)` **set_bone_name**(bone_idx: `int`, name: `String`)

Sets the bone name, `name`, for the bone at `bone_idx`.

`void (No return value.)` **set_bone_parent**(bone_idx: `int`, parent_idx: `int`)

Sets the bone index `parent_idx` as the parent of the bone at `bone_idx`. If -1, then bone has no parent.

**Note:** `parent_idx` must be less than `bone_idx`.

`void (No return value.)` **set_bone_pose**(bone_idx: `int`, pose: `Transform3D`)

Sets the pose transform, `pose`, for the bone at `bone_idx`.

`void (No return value.)` **set_bone_pose_position**(bone_idx: `int`, position: `Vector3`)
...
`void (No return value.)` **set_bone_pose_rotation**(bone_idx: `int`, rotation: `Quaternion`)
...
`void (No return value.)` **set_bone_pose_scale**(bone_idx: `int`, scale: `Vector3`)

Sets the pose scale of the bone at `bone_idx` to `scale`.

`void (No return value.)` **set_bone_rest**(bone_idx: `int`, rest: `Transform3D`)

Sets the rest transform for bone `bone_idx`.

`void (No return value.)` **unparent_bone_and_rest**(bone_idx: `int`)
