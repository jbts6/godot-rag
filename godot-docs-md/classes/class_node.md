...
- `void (No return value.)` **set_auto_translate_mode**(value: `AutoTranslateMode`)
...
- `void (No return value.)` **set_editor_description**(value: `String`)
...
- `void (No return value.)` **set_owner**(value: `Node`)
- `Node` **get_owner**()
...
- `void (No return value.)` **set_physics_interpolation_mode**(value: `PhysicsInterpolationMode`)
...
- `void (No return value.)` **set_process_mode**(value: `ProcessMode`)
...
- `void (No return value.)` **set_physics_process_priority**(value: `int`)
...
- `void (No return value.)` **set_process_priority**(value: `int`)
...
- `void (No return value.)` **set_process_thread_group**(value: `ProcessThreadGroup`)
...
- `void (No return value.)` **set_process_thread_group_order**(value: `int`)
- `int` **get_process_thread_group_order**()

Change the process thread group order. Groups with a lesser order will process before groups with a greater order. This is useful when a large amount of nodes process in sub thread and, afterwards, another group wants to collect their result in the main thread, as an example.

`BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ProcessThreadMessages`\] **process_thread_messages**

- `void (No return value.)` **set_process_thread_messages**(value: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ProcessThreadMessages`\])
- `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`ProcessThreadMessages`\] **get_process_thread_messages**()
...
- `void (No return value.)` **set_scene_file_path**(value: `String`)
...
- `void (No return value.)` **set_unique_name_in_owner**(value: `bool`)
...
`void (No return value.)` **\_enter_tree**() `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_exit_tree**() `virtual (This method should typically be overridden by the user to have any effect.)`
...
`PackedStringArray` **\_get_accessibility_configuration_warnings**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`PackedStringArray` **\_get_configuration_warnings**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`RID` **\_get_focused_accessibility_element**() `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`void (No return value.)` **\_input**(event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_physics_process**(delta: `float`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_process**(delta: `float`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_ready**() `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_shortcut_input**(event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_unhandled_input**(event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **\_unhandled_key_input**(event: `InputEvent`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **add_child**(node: `Node`, force_readable_name: `bool` = false, internal: `InternalMode` = 0)
...
`void (No return value.)` **add_sibling**(sibling: `Node`, force_readable_name: `bool` = false)
...
`void (No return value.)` **add_to_group**(group: `StringName`, persistent: `bool` = false)
...
`Array`\[`int`\] **get_orphan_node_ids**() `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **move_child**(child_node: `Node`, to_index: `int`)
...
`void (No return value.)` **notify_deferred_thread_group**(what: `int`)

Similar to `call_deferred_thread_group()`, but for notifications.

`void (No return value.)` **notify_thread_safe**(what: `int`)

Similar to `call_thread_safe()`, but for notifications.

`void (No return value.)` **print_orphan_nodes**() `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`

Prints all orphan nodes (nodes outside the `SceneTree`). Useful for debugging.

**Note:** This method only works in debug builds. It does nothing in a project exported in release mode.

`void (No return value.)` **print_tree**()
...
`void (No return value.)` **print_tree_pretty**()
...
`void (No return value.)` **propagate_call**(method: `StringName`, args: `Array` = \[\], parent_first: `bool` = false)
...
`void (No return value.)` **propagate_notification**(what: `int`)

Calls `Object.notification()` with `what` on this node and all of its children, recursively.

`void (No return value.)` **queue_accessibility_update**()

Queues an accessibility information update for this node.

`void (No return value.)` **queue_free**()
...
`void (No return value.)` **remove_child**(node: `Node`)
...
`void (No return value.)` **remove_from_group**(group: `StringName`)
...
`void (No return value.)` **reparent**(new_parent: `Node`, keep_global_transform: `bool` = true)
...
`void (No return value.)` **replace_by**(node: `Node`, keep_groups: `bool` = false)
...
`void (No return value.)` **request_ready**()
...
`void (No return value.)` **reset_physics_interpolation**()
...
`void (No return value.)` **rpc_config**(method: `StringName`, config: `Variant`)
...
`void (No return value.)` **set_deferred_thread_group**(property: `StringName`, value: `Variant`)

Similar to `call_deferred_thread_group()`, but for setting properties.

`void (No return value.)` **set_display_folded**(fold: `bool`)
...
`void (No return value.)` **set_editable_instance**(node: `Node`, is_editable: `bool`)
...
`void (No return value.)` **set_multiplayer_authority**(id: `int`, recursive: `bool` = true)
...
`void (No return value.)` **set_physics_process**(enable: `bool`)
...
`void (No return value.)` **set_physics_process_internal**(enable: `bool`)
...
`void (No return value.)` **set_process**(enable: `bool`)
...
`void (No return value.)` **set_process_input**(enable: `bool`)
...
`void (No return value.)` **set_process_internal**(enable: `bool`)
...
`void (No return value.)` **set_process_shortcut_input**(enable: `bool`)

If set to `true`, enables shortcut processing for this node.

**Note:** If `_shortcut_input()` is overridden, this will be automatically enabled before `_ready()` is called.

`void (No return value.)` **set_process_unhandled_input**(enable: `bool`)
...
`void (No return value.)` **set_process_unhandled_key_input**(enable: `bool`)

If set to `true`, enables unhandled key input processing.

**Note:** If `_unhandled_key_input()` is overridden, this will be automatically enabled before `_ready()` is called.

`void (No return value.)` **set_scene_instance_load_placeholder**(load_placeholder: `bool`)
...
`void (No return value.)` **set_thread_safe**(property: `StringName`, value: `Variant`)

Similar to `call_thread_safe()`, but for setting properties.

`void (No return value.)` **set_translation_domain_inherited**()
...
`void (No return value.)` **update_configuration_warnings**()
