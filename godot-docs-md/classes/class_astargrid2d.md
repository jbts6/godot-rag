...
- `void (No return value.)` **set_cell_shape**(value: `CellShape`)
...
- `void (No return value.)` **set_cell_size**(value: `Vector2`)
...
- `void (No return value.)` **set_default_compute_heuristic**(value: `Heuristic`)
...
- `void (No return value.)` **set_default_estimate_heuristic**(value: `Heuristic`)
...
- `void (No return value.)` **set_diagonal_mode**(value: `DiagonalMode`)
...
- `void (No return value.)` **set_jumping_enabled**(value: `bool`)
...
- `void (No return value.)` **set_region**(value: `Rect2i`)
- `Rect2i` **get_region**()
...
`float` **\_compute_cost**(from_id: `Vector2i`, to_id: `Vector2i`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`

Called when computing the cost between two connected points.

Note that this function is hidden in the default **AStarGrid2D** class.

`float` **\_estimate_cost**(from_id: `Vector2i`, end_id: `Vector2i`) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`void (No return value.)` **fill_solid_region**(region: `Rect2i`, solid: `bool` = true)
...
`void (No return value.)` **fill_weight_scale_region**(region: `Rect2i`, weight_scale: `float`)
...
`void (No return value.)` **set_point_solid**(id: `Vector2i`, solid: `bool` = true)

Disables or enables the specified point for pathfinding. Useful for making an obstacle. By default, all points are enabled.

**Note:** Calling `update()` is not needed after the call of this function.

`void (No return value.)` **set_point_weight_scale**(id: `Vector2i`, weight_scale: `float`)
