...
- `void (No return value.)` **set_include_hidden**(value: `bool`)
...
- `void (No return value.)` **set_include_navigational**(value: `bool`)
...
`Error` **copy_absolute**(from: `String`, to: `String`, chmod_flags: `int` = -1) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`DirAccess` **create_temp**(prefix: `String` = "", keep: `bool` = false) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`bool` **dir_exists_absolute**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`PackedStringArray` **get_directories_at**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`int` **get_drive_count**() `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`String` **get_drive_label**(idx: `int`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`String` **get_drive_name**(idx: `int`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`PackedStringArray` **get_files_at**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **list_dir_end**()
...
`Error` **make_dir_absolute**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`Error` **make_dir_recursive_absolute**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`

Static version of `make_dir_recursive()`. Supports only absolute paths.

`DirAccess` **open**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`Error` **remove_absolute**(path: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`Error` **rename_absolute**(from: `String`, to: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
