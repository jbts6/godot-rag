...
`void (No return value.)` **free_spatial_context**(spatial_context: `RID`)
...
`void (No return value.)` **free_spatial_entity**(entity: `RID`)

Frees an entity previously created when calling `add_spatial_entity()` or `make_spatial_entity()`.

`void (No return value.)` **free_spatial_snapshot**(spatial_snapshot: `RID`)
