...
`TLSOptions` **client**(trusted_chain: `X509Certificate` = null, common_name_override: `String` = "") `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`TLSOptions` **client_unsafe**(trusted_chain: `X509Certificate` = null) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`TLSOptions` **server**(key: `CryptoKey`, certificate: `X509Certificate`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
