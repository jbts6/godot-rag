...
- `void (No return value.)` **set_discover_ipv6**(value: `bool`)
...
- `void (No return value.)` **set_discover_local_port**(value: `int`)
...
- `void (No return value.)` **set_discover_multicast_if**(value: `String`)
...
`void (No return value.)` **add_device**(device: `UPNPDevice`)
...
`void (No return value.)` **clear_devices**()
...
`void (No return value.)` **remove_device**(index: `int`)

Removes the device at `index` from the list of discovered devices.

`void (No return value.)` **set_device**(index: `int`, device: `UPNPDevice`)
