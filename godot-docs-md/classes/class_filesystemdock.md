...
`void (No return value.)` **add_resource_tooltip_plugin**(plugin: `EditorResourceTooltipPlugin`)

Registers a new `EditorResourceTooltipPlugin`.

`void (No return value.)` **navigate_to_path**(path: `String`)

Sets the given `path` as currently selected, ensuring that the selected file/directory is visible.

`void (No return value.)` **remove_resource_tooltip_plugin**(plugin: `EditorResourceTooltipPlugin`)
