...
`void (No return value.)` **call_deferred**(...) `vararg` `const`
...
`Callable` **create**(variant: `Variant`, method: `StringName`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`void (No return value.)` **rpc**(...) `vararg` `const`
...
`void (No return value.)` **rpc_id**(peer_id: `int`, ...) `vararg` `const`
