...
`void (No return value.)` **agent_set_avoidance_callback**(agent: `RID`, callback: `Callable`)
...
`void (No return value.)` **agent_set_avoidance_enabled**(agent: `RID`, enabled: `bool`)

If `enabled` is `true`, the provided `agent` calculates avoidance.

`void (No return value.)` **agent_set_avoidance_layers**(agent: `RID`, layers: `int`)

Set the agent's `avoidance_layers` bitmask.

`void (No return value.)` **agent_set_avoidance_mask**(agent: `RID`, mask: `int`)

Set the agent's `avoidance_mask` bitmask.

`void (No return value.)` **agent_set_avoidance_priority**(agent: `RID`, priority: `float`)
...
`void (No return value.)` **agent_set_height**(agent: `RID`, height: `float`)

Updates the provided `agent` `height`.

`void (No return value.)` **agent_set_map**(agent: `RID`, map: `RID`)

Puts the agent in the map.

`void (No return value.)` **agent_set_max_neighbors**(agent: `RID`, count: `int`)

Sets the maximum number of other agents the agent takes into account in the navigation. The larger this number, the longer the running time of the simulation. If the number is too low, the simulation will not be safe.

`void (No return value.)` **agent_set_max_speed**(agent: `RID`, max_speed: `float`)

Sets the maximum speed of the agent. Must be positive.

`void (No return value.)` **agent_set_neighbor_distance**(agent: `RID`, distance: `float`)

Sets the maximum distance to other agents this agent takes into account in the navigation. The larger this number, the longer the running time of the simulation. If the number is too low, the simulation will not be safe.

`void (No return value.)` **agent_set_paused**(agent: `RID`, paused: `bool`)
...
`void (No return value.)` **agent_set_position**(agent: `RID`, position: `Vector3`)

Sets the position of the agent in world space.

`void (No return value.)` **agent_set_radius**(agent: `RID`, radius: `float`)

Sets the radius of the agent.

`void (No return value.)` **agent_set_time_horizon_agents**(agent: `RID`, time_horizon: `float`)

The minimal amount of time for which the agent's velocities that are computed by the simulation are safe with respect to other agents. The larger this number, the sooner this agent will respond to the presence of other agents, but the less freedom this agent has in choosing its velocities. A too high value will slow down agents movement considerably. Must be positive.

`void (No return value.)` **agent_set_time_horizon_obstacles**(agent: `RID`, time_horizon: `float`)

The minimal amount of time for which the agent's velocities that are computed by the simulation are safe with respect to static avoidance obstacles. The larger this number, the sooner this agent will respond to the presence of static avoidance obstacles, but the less freedom this agent has in choosing its velocities. A too high value will slow down agents movement considerably. Must be positive.

`void (No return value.)` **agent_set_use_3d_avoidance**(agent: `RID`, enabled: `bool`)
...
`void (No return value.)` **agent_set_velocity**(agent: `RID`, velocity: `Vector3`)
...
`void (No return value.)` **agent_set_velocity_forced**(agent: `RID`, velocity: `Vector3`)
...
`void (No return value.)` **bake_from_source_geometry_data**(navigation_mesh: `NavigationMesh`, source_geometry_data: `NavigationMeshSourceGeometryData3D`, callback: `Callable` = Callable())
...
`void (No return value.)` **bake_from_source_geometry_data_async**(navigation_mesh: `NavigationMesh`, source_geometry_data: `NavigationMeshSourceGeometryData3D`, callback: `Callable` = Callable())
...
`void (No return value.)` **link_set_bidirectional**(link: `RID`, bidirectional: `bool`)

Sets whether this `link` can be travelled in both directions.

`void (No return value.)` **link_set_enabled**(link: `RID`, enabled: `bool`)

If `enabled` is `true`, the specified `link` will contribute to its current navigation map.

`void (No return value.)` **link_set_end_position**(link: `RID`, position: `Vector3`)

Sets the exit position for the `link`.

`void (No return value.)` **link_set_enter_cost**(link: `RID`, enter_cost: `float`)

Sets the `enter_cost` for this `link`.

`void (No return value.)` **link_set_map**(link: `RID`, map: `RID`)

Sets the navigation map `RID` for the link.

`void (No return value.)` **link_set_navigation_layers**(link: `RID`, navigation_layers: `int`)

Set the links's navigation layers. This allows selecting links from a path request (when using `map_get_path()`).

`void (No return value.)` **link_set_owner_id**(link: `RID`, owner_id: `int`)

Set the `ObjectID` of the object which manages this link.

`void (No return value.)` **link_set_start_position**(link: `RID`, position: `Vector3`)

Sets the entry position for this `link`.

`void (No return value.)` **link_set_travel_cost**(link: `RID`, travel_cost: `float`)
...
`void (No return value.)` **map_force_update**(map: `RID`)
...
`void (No return value.)` **map_set_active**(map: `RID`, active: `bool`)

Sets the map active.

`void (No return value.)` **map_set_cell_height**(map: `RID`, cell_height: `float`)
...
`void (No return value.)` **map_set_cell_size**(map: `RID`, cell_size: `float`)
...
`void (No return value.)` **map_set_edge_connection_margin**(map: `RID`, margin: `float`)

Set the map edge connection margin used to weld the compatible region edges.

`void (No return value.)` **map_set_link_connection_radius**(map: `RID`, radius: `float`)

Set the map's link connection radius used to connect links to navigation polygons.

`void (No return value.)` **map_set_merge_rasterizer_cell_scale**(map: `RID`, scale: `float`)

Set the map's internal merge rasterizer cell scale used to control merging sensitivity.

`void (No return value.)` **map_set_up**(map: `RID`, up: `Vector3`)

Sets the map up direction.

`void (No return value.)` **map_set_use_async_iterations**(map: `RID`, enabled: `bool`)

If `enabled` is `true` the `map` synchronization uses an async process that runs on a background thread.

`void (No return value.)` **map_set_use_edge_connections**(map: `RID`, enabled: `bool`)
...
`void (No return value.)` **obstacle_set_avoidance_enabled**(obstacle: `RID`, enabled: `bool`)

If `enabled` is `true`, the provided `obstacle` affects avoidance using agents.

`void (No return value.)` **obstacle_set_avoidance_layers**(obstacle: `RID`, layers: `int`)

Set the obstacles's `avoidance_layers` bitmask.

`void (No return value.)` **obstacle_set_height**(obstacle: `RID`, height: `float`)
...
`void (No return value.)` **obstacle_set_map**(obstacle: `RID`, map: `RID`)

Assigns the `obstacle` to a navigation map.

`void (No return value.)` **obstacle_set_paused**(obstacle: `RID`, paused: `bool`)

If `paused` is `true` the specified `obstacle` will not be processed. For example, it will no longer affect avoidance velocities.

`void (No return value.)` **obstacle_set_position**(obstacle: `RID`, position: `Vector3`)

Updates the `position` in world space for the `obstacle`.

`void (No return value.)` **obstacle_set_radius**(obstacle: `RID`, radius: `float`)

Sets the radius of the dynamic obstacle.

`void (No return value.)` **obstacle_set_use_3d_avoidance**(obstacle: `RID`, enabled: `bool`)

Sets if the `obstacle` uses the 2D avoidance or the 3D avoidance while avoidance is enabled.

`void (No return value.)` **obstacle_set_velocity**(obstacle: `RID`, velocity: `Vector3`)
...
`void (No return value.)` **obstacle_set_vertices**(obstacle: `RID`, vertices: `PackedVector3Array`)

Sets the outline vertices for the obstacle. If the vertices are winded in clockwise order agents will be pushed in by the obstacle, else they will be pushed out.

`void (No return value.)` **parse_source_geometry_data**(navigation_mesh: `NavigationMesh`, source_geometry_data: `NavigationMeshSourceGeometryData3D`, root_node: `Node`, callback: `Callable` = Callable())
...
`void (No return value.)` **query_path**(parameters: `NavigationPathQueryParameters3D`, result: `NavigationPathQueryResult3D`, callback: `Callable` = Callable())
...
`void (No return value.)` **region_bake_navigation_mesh**(navigation_mesh: `NavigationMesh`, root_node: `Node`)
...
`void (No return value.)` **region_set_enabled**(region: `RID`, enabled: `bool`)

If `enabled` is `true`, the specified `region` will contribute to its current navigation map.

`void (No return value.)` **region_set_enter_cost**(region: `RID`, enter_cost: `float`)

Sets the `enter_cost` for this `region`.

`void (No return value.)` **region_set_map**(region: `RID`, map: `RID`)

Sets the map for the region.

`void (No return value.)` **region_set_navigation_layers**(region: `RID`, navigation_layers: `int`)

Set the region's navigation layers. This allows selecting regions from a path request (when using `map_get_path()`).

`void (No return value.)` **region_set_navigation_mesh**(region: `RID`, navigation_mesh: `NavigationMesh`)

Sets the navigation mesh for the region.

`void (No return value.)` **region_set_owner_id**(region: `RID`, owner_id: `int`)

Set the `ObjectID` of the object which manages this region.

`void (No return value.)` **region_set_transform**(region: `RID`, transform: `Transform3D`)

Sets the global transformation for the region.

`void (No return value.)` **region_set_travel_cost**(region: `RID`, travel_cost: `float`)

Sets the `travel_cost` for this `region`.

`void (No return value.)` **region_set_use_async_iterations**(region: `RID`, enabled: `bool`)

If `enabled` is `true` the `region` uses an async synchronization process that runs on a background thread.

`void (No return value.)` **region_set_use_edge_connections**(region: `RID`, enabled: `bool`)
...
`void (No return value.)` **set_debug_enabled**(enabled: `bool`)
...
`void (No return value.)` **source_geometry_parser_set_callback**(parser: `RID`, callback: `Callable`)
