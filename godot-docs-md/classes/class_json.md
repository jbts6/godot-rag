...
- `void (No return value.)` **set_data**(value: `Variant`)
- `Variant` **get_data**()
...
`Variant` **from_native**(variant: `Variant`, full_objects: `bool` = false) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`Variant` **parse_string**(json_string: `String`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`

Attempts to parse the `json_string` provided and returns the parsed data. Returns `null` if parse failed.

`String` **stringify**(data: `Variant`, indent: `String` = "", sort_keys: `bool` = true, full_precision: `bool` = false) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
...
`Variant` **to_native**(json: `Variant`, allow_objects: `bool` = false) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
