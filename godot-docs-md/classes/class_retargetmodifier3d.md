...
`BitField (This value is an integer composed as a bitmask of the following flags.)`\[`TransformFlag`\] **enable** = `7`

- `void (No return value.)` **set_enable_flags**(value: `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`TransformFlag`\])
- `BitField (This value is an integer composed as a bitmask of the following flags.)`\[`TransformFlag`\] **get_enable_flags**()

Flags to control the process of the transform elements individually when `use_global_pose` is disabled.

`SkeletonProfile` **profile**

- `void (No return value.)` **set_profile**(value: `SkeletonProfile`)
...
- `void (No return value.)` **set_use_global_pose**(value: `bool`)
...
`void (No return value.)` **set_position_enabled**(enabled: `bool`)

Sets `TRANSFORM_FLAG_POSITION` into `enable`.

`void (No return value.)` **set_rotation_enabled**(enabled: `bool`)

Sets `TRANSFORM_FLAG_ROTATION` into `enable`.

`void (No return value.)` **set_scale_enabled**(enabled: `bool`)
