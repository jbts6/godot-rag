...
`void (No return value.)` **connect_node**(input_node: `StringName`, input_index: `int`, output_node: `StringName`)

Connects the output of an `AnimationNode` as input for another `AnimationNode`, at the input port specified by `input_index`.

`void (No return value.)` **disconnect_node**(input_node: `StringName`, input_index: `int`)
