...
`Variant` **interpolate_value**(initial_value: `Variant`, delta_value: `Variant`, elapsed_time: `float`, duration: `float`, trans_type: `TransitionType`, ease_type: `EaseType`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
