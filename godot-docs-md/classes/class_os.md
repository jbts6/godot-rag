...
- `void (No return value.)` **set_delta_smoothing**(value: `bool`)
...
- `void (No return value.)` **set_low_processor_usage_mode**(value: `bool`)
...
- `void (No return value.)` **set_low_processor_usage_mode_sleep_usec**(value: `int`)
...
`void (No return value.)` **add_logger**(logger: `Logger`)

Add a custom logger to intercept the internal message stream.

`void (No return value.)` **alert**(text: `String`, title: `String` = "Alert!")
...
`void (No return value.)` **close_midi_inputs**()
...
`void (No return value.)` **crash**(message: `String`)
...
`void (No return value.)` **delay_msec**(msec: `int`) `const`
...
`void (No return value.)` **delay_usec**(usec: `int`) `const`
...
`void (No return value.)` **open_midi_inputs**()
...
`void (No return value.)` **remove_logger**(logger: `Logger`)
...
`void (No return value.)` **revoke_granted_permissions**()

On macOS (sandboxed applications only), this function clears list of user selected folders accessible to the application.

`void (No return value.)` **set_environment**(variable: `String`, value: `String`) `const`
...
`void (No return value.)` **set_restart_on_exit**(restart: `bool`, arguments: `PackedStringArray` = PackedStringArray())
...
`void (No return value.)` **set_use_file_access_save_and_swap**(enabled: `bool`)
...
`void (No return value.)` **unset_environment**(variable: `String`) `const`
