...
- `void (No return value.)` **set_body_type**(value: `String`)
...
- `void (No return value.)` **set_inertia_diagonal**(value: `Vector3`)
...
- `void (No return value.)` **set_inertia_orientation**(value: `Quaternion`)
...
- `void (No return value.)` **set_inertia_tensor**(value: `Basis`)
...
`GLTFPhysicsBody` **from_dictionary**(dictionary: `Dictionary`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`

Creates a new GLTFPhysicsBody instance by parsing the given `Dictionary` in the `OMI_physics_body` glTF extension format.

`GLTFPhysicsBody` **from_node**(body_node: `CollisionObject3D`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
