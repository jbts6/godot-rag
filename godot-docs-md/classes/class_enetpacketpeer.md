...
`void (No return value.)` **peer_disconnect**(data: `int` = 0)
...
`void (No return value.)` **peer_disconnect_later**(data: `int` = 0)
...
`void (No return value.)` **peer_disconnect_now**(data: `int` = 0)
...
`void (No return value.)` **ping_interval**(ping_interval: `int`)
...
`void (No return value.)` **set_timeout**(timeout: `int`, timeout_min: `int`, timeout_max: `int`)
...
`void (No return value.)` **throttle_configure**(interval: `int`, acceleration: `int`, deceleration: `int`)
