...
`Array`\[`PackedStringArray`\] **\_customize_strings**(strings: `Array`\[`PackedStringArray`\]) `virtual (This method should typically be overridden by the user to have any effect.)` `const`
...
`Array`\[`PackedStringArray`\] **\_parse_file**(path: `String`) `virtual (This method should typically be overridden by the user to have any effect.)`
