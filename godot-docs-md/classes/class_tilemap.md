...
- `void (No return value.)` **set_collision_animatable**(value: `bool`)
...
- `void (No return value.)` **set_collision_visibility_mode**(value: `VisibilityMode`)
...
- `void (No return value.)` **set_navigation_visibility_mode**(value: `VisibilityMode`)
...
- `void (No return value.)` **set_rendering_quadrant_size**(value: `int`)
...
- `void (No return value.)` **set_tileset**(value: `TileSet`)
- `TileSet` **get_tileset**()
...
`void (No return value.)` **\_tile_data_runtime_update**(layer: `int`, coords: `Vector2i`, tile_data: `TileData`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`bool` **\_use_tile_data_runtime_update**(layer: `int`, coords: `Vector2i`) `virtual (This method should typically be overridden by the user to have any effect.)`
...
`void (No return value.)` **add_layer**(to_position: `int`)
...
`void (No return value.)` **clear_layer**(layer: `int`)

Clears all cells on the given layer.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **erase_cell**(layer: `int`, coords: `Vector2i`)

Erases the cell on layer `layer` at coordinates `coords`.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **fix_invalid_tiles**()

Clears cells that do not exist in the tileset.

`void (No return value.)` **force_update**(layer: `int` = -1)
...
`void (No return value.)` **move_layer**(layer: `int`, to_position: `int`)

Moves the layer at index `layer` to the given position `to_position` in the array.

`void (No return value.)` **notify_runtime_tile_data_update**(layer: `int` = -1)
...
`void (No return value.)` **remove_layer**(layer: `int`)

Removes the layer at index `layer`.

`void (No return value.)` **set_cell**(layer: `int`, coords: `Vector2i`, source_id: `int` = -1, atlas_coords: `Vector2i` = Vector2i(-1, -1), alternative_tile: `int` = 0)
...
`void (No return value.)` **set_cells_terrain_connect**(layer: `int`, cells: `Array`\[`Vector2i`\], terrain_set: `int`, terrain: `int`, ignore_empty_terrains: `bool` = true)
...
`void (No return value.)` **set_cells_terrain_path**(layer: `int`, path: `Array`\[`Vector2i`\], terrain_set: `int`, terrain: `int`, ignore_empty_terrains: `bool` = true)
...
`void (No return value.)` **set_layer_enabled**(layer: `int`, enabled: `bool`)

Enables or disables the layer `layer`. A disabled layer is not processed at all (no rendering, no physics, etc.).

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **set_layer_modulate**(layer: `int`, modulate: `Color`)

Sets a layer's color. It will be multiplied by tile's color and TileMap's modulate.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **set_layer_name**(layer: `int`, name: `String`)

Sets a layer's name. This is mostly useful in the editor.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **set_layer_navigation_enabled**(layer: `int`, enabled: `bool`)
...
`void (No return value.)` **set_layer_navigation_map**(layer: `int`, map: `RID`)
...
`void (No return value.)` **set_layer_y_sort_enabled**(layer: `int`, y_sort_enabled: `bool`)
...
`void (No return value.)` **set_layer_y_sort_origin**(layer: `int`, y_sort_origin: `int`)
...
`void (No return value.)` **set_layer_z_index**(layer: `int`, z_index: `int`)

Sets a layers Z-index value. This Z-index is added to each tile's Z-index value.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **set_navigation_map**(layer: `int`, map: `RID`)

**Deprecated:** Use `set_layer_navigation_map()` instead.

Assigns `map` as a `NavigationServer2D` navigation map for the specified TileMap layer `layer`.

`void (No return value.)` **set_pattern**(layer: `int`, position: `Vector2i`, pattern: `TileMapPattern`)

Paste the given `TileMapPattern` at the given `position` and `layer` in the tile map.

If `layer` is negative, the layers are accessed from the last one.

`void (No return value.)` **update_internals**()
