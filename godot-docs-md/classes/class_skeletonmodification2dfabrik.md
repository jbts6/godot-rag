...
- `void (No return value.)` **set_fabrik_data_chain_length**(value: `int`)
...
`void (No return value.)` **set_fabrik_joint_bone2d_node**(joint_idx: `int`, bone2d_nodepath: `NodePath`)

Sets the `Bone2D` node assigned to the FABRIK joint at `joint_idx`.

`void (No return value.)` **set_fabrik_joint_bone_index**(joint_idx: `int`, bone_idx: `int`)
...
`void (No return value.)` **set_fabrik_joint_magnet_position**(joint_idx: `int`, magnet_position: `Vector2`)

Sets the magnet position vector for the joint at `joint_idx`.

`void (No return value.)` **set_fabrik_joint_use_target_rotation**(joint_idx: `int`, use_target_rotation: `bool`)
