...
- `void (No return value.)` **set_ccdik_data_chain_length**(value: `int`)
...
- `void (No return value.)` **set_tip_node**(value: `NodePath`)
...
`void (No return value.)` **set_ccdik_joint_bone2d_node**(joint_idx: `int`, bone2d_nodepath: `NodePath`)

Sets the `Bone2D` node assigned to the CCDIK joint at `joint_idx`.

`void (No return value.)` **set_ccdik_joint_bone_index**(joint_idx: `int`, bone_idx: `int`)
...
`void (No return value.)` **set_ccdik_joint_constraint_angle_invert**(joint_idx: `int`, invert: `bool`)
...
`void (No return value.)` **set_ccdik_joint_constraint_angle_max**(joint_idx: `int`, angle_max: `float`)

Sets the maximum angle constraint for the joint at `joint_idx`.

`void (No return value.)` **set_ccdik_joint_constraint_angle_min**(joint_idx: `int`, angle_min: `float`)

Sets the minimum angle constraint for the joint at `joint_idx`.

`void (No return value.)` **set_ccdik_joint_enable_constraint**(joint_idx: `int`, enable_constraint: `bool`)
...
`void (No return value.)` **set_ccdik_joint_rotate_from_joint**(joint_idx: `int`, rotate_from_joint: `bool`)
