...
- `void (No return value.)` **set_end_position**(value: `Vector3`)
...
`void (No return value.)` **set_global_end_position**(position: `Vector3`)

Sets the `end_position` that is relative to the link from a global `position`.

`void (No return value.)` **set_global_start_position**(position: `Vector3`)
