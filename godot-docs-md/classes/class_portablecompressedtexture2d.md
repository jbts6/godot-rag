...
- `void (No return value.)` **set_keep_compressed_buffer**(value: `bool`)
...
- `void (No return value.)` **set_size_override**(value: `Vector2`)
...
`void (No return value.)` **create_from_image**(image: `Image`, compression_mode: `CompressionMode`, normal_map: `bool` = false, lossy_quality: `float` = 0.8)
...
`bool` **is_keeping_all_compressed_buffers**() `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`

Returns `true` if the flag is overridden for all textures of this type.

`void (No return value.)` **set_basisu_compressor_params**(uastc_level: `int`, rdo_quality_loss: `float`)
...
`void (No return value.)` **set_keep_all_compressed_buffers**(keep: `bool`) `static (This method doesn't need an instance to be called, so it can be called directly using the class name.)`
