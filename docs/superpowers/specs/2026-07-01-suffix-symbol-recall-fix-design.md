...
- **[风险] 已发布 DB 与新代码不兼容**：用户升级 `godot-rag` 后不重建 DB，suffix recall 仍失效（但不会崩）。→ 缓解：CHANGELOG/release notes 注明「升级后请重建索引」；代码层面无 schema 变更，旧 DB 行为退化到「suffix 不命中」即原本状态，无回归。
