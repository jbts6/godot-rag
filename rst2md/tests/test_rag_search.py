...
def test_symbol_query_returns_exact_method_first():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert results[0].symbol == "StringName.is_valid_filename"
        assert results[0].path == "classes/class_stringname.md"

def test_natural_language_alias_returns_expected_symbol():
    with tempfile.TemporaryDirectory() as tmp:
        docs = Path(tmp) / "docs"
        classes = docs / "classes"
        ...
        assert results
        assert results[0].symbol == "Node.add_child"


def _build_fts_score_db(tmp):
    docs = Path(tmp) / "docs"
    docs.mkdir()
    classes = docs / "classes"
    classes.mkdir()
...
def test_fts_formula_produces_varied_scores():
    ...
    assert len(scores) > 3, "Formula should produce varied scores"


def _build_fts_escape_db(tmp):
    docs = Path(tmp) / "docs"
    docs.mkdir()
    classes = docs / "classes"
    classes.mkdir()
...
def test_colon_in_query_still_returns_fts_results():
    """Queries with FTS5 special chars (e.g. ':') should not silently
    suppress full-text results."""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_fts_escape_db(tmp)
        ...
        assert "is_stopped" in texts

def test_plain_fts_query_returns_results():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_fts_escape_db(tmp)
        results = search_database(db_path, "stopped timer", limit=3)
        texts = " ".join(r.text for r in results)
        assert "stopped" in texts.lower()


def _build_doctype_db(tmp):
    docs = Path(tmp) / "docs"
    docs.mkdir()
...
def test_search_all_returns_both_types():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_doctype_db(tmp)
...
def test_search_class_only():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_doctype_db(tmp)
        results = search_database(db_path, "timer stopped", limit=10, doc_types=["class"])
        doc_types = {r.doc_type for r in results}
        assert "tutorial" not in doc_types

def test_search_tutorial_only():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_doctype_db(tmp)
        ...
        doc_types = {r.doc_type for r in results}
        assert "class" not in doc_types

def test_empty_doc_types_treated_as_none():
...
def test_import_extract_inherits():
    from rag.relations import extract_inherits
    assert extract_inherits("no inherits here") == []

def test_import_inherits_re():
    from rag.relations import INHERITS_RE
    assert INHERITS_RE.search("**Inherits:** `Object`")

def test_import_build_chunk_relations():
    from rag.relations import build_chunk_relations
    import inspect
    assert callable(build_chunk_relations)
    assert len(inspect.signature(build_chunk_relations).parameters) == 1

def test_extract_inherits_finds_backticked_names():
    from rag.relations import extract_inherits
    result = extract_inherits("**Inherits:** `Node` and `Object`")
    assert result == ["Node", "Object"]


def _build_chunk_relation_db(tmp):
    docs = Path(tmp) / "docs"
    docs.mkdir()
    classes = docs / "classes"
    classes.mkdir()
...
def test_relation_table_exists():
    import sqlite3
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_chunk_relation_db(tmp)
        conn = sqlite3.connect(str(db_path))
        tables = [r[0] for r in conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table'"
        ).fetchall()]
        conn.close()
        assert "chunk_relations" in tables

def test_parent_relations_exist():
    import sqlite3
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_chunk_relation_db(tmp)
        conn = sqlite3.connect(str(db_path))
        ...
        conn.close()
        assert count > 0, "Should have parent relations"

def test_inherits_relations_exist():
    import sqlite3
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_chunk_relation_db(tmp)
        conn = sqlite3.connect(str(db_path))
        ...
        conn.close()
        assert count > 0, "Should have inherits relations"


def test_dotted_symbol_splits_to_tokens():
...
def test_plain_query_unchanged():
    ...
    assert result == "scene transition"

def test_special_chars_quoted():
    from rag.store import _smart_tokenize
    result = _smart_tokenize("Node::add")
    # Colons should be quoted
    assert '"Node::add"' in result


def _build_graph_expansion_db(tmp):
    docs = Path(tmp) / "docs"
    docs.mkdir()
    classes = docs / "classes"
    classes.mkdir()
...
def test_expand_returns_parent_chunk():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "Node.add_child" in symbols
        # Parent (Node class_summary) should be in results
        assert "Node" in symbols

def test_no_expand_returns_only_direct():
...
def test_expanded_results_have_distance():
    """Expanded results should have distance=1."""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_graph_expansion_db(tmp)
        results = search_database(db_path, "add_child", limit=5, expand_graph=True)
        expanded = [r for r in results if r.distance == 1]
        assert len(expanded) > 0, "Should have expanded results with distance=1"

def test_expanded_results_have_relation_type():
    """Expanded results should have a relation_type."""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_graph_expansion_db(tmp)
        results = search_database(db_path, "add_child", limit=5, expand_graph=True)
        expanded = [r for r in results if r.relation_type]
        assert len(expanded) > 0, "Should have expanded results with relation_type"


def _build_addon_db(tmp):
...
def test_addon_search_respects_no_expand():
    """search_database with expand_graph=False should not expand results."""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_addon_db(tmp)
        ...
        assert len(results_no_expand) <= len(results_expand)
        # Without expansion, all results should have distance=0
        for r in results_no_expand:
            assert r.distance == 0, "No-expand results should have distance=0"

def test_cli_addon_search_no_expand_flag():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("rag.cli.search_database") as mock_search:
            ...
            assert call_kwargs.kwargs.get("expand_graph", False) == False, "search_database should be called with expand_graph=False when --no-expand is set"
...
def test_cli_search_alias_works():
...
def test_build_shows_progress():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "Building database" in output


def test_clean_chunk_text_importable_from_store_facade():
    """clean_chunk_text should remain importable from rag.store."""
    from rag.store import clean_chunk_text

    assert clean_chunk_text("classref-test\n`Node<class_Node>`") == "`Node`"
...
def test_get_stats_returns_expected_keys():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_stats_db(tmp)
        stats = get_stats(db_path)

        assert "chunks" in stats
        assert "symbols" in stats
        assert "relations" in stats
        assert "addons" in stats

def test_get_stats_returns_correct_chunk_count():
...
def test_search_result_has_snippet_field():
...
def test_search_database_returns_snippet():
    """search_database should return results with snippet field."""
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_snippet_db(tmp)
...
def test_ranking_signal_defaults():
    from rag.models import RankingSignal

    signal = RankingSignal(name="symbol_recall.exact", weight=100.0)
    assert signal.name == "symbol_recall.exact"
    assert signal.weight == 100.0
    assert signal.value is None
    assert signal.details == {}

def test_ranking_signal_details_not_shared_across_instances():
    from rag.models import RankingSignal

    a = RankingSignal(name="x", weight=1.0)
    b = RankingSignal(name="y", weight=2.0)
    a.details["k"] = "v"
    assert "k" not in b.details, "details dict must not be shared across instances"

def test_search_result_ranking_signals_default_empty():
    from rag.models import SearchResult
...
def test_search_result_positional_construction_still_works():
...
def test_addon_search_no_expand_flag():
...
def test_cli_search_class_help():
    result = subprocess.run(
...
def test_cli_search_tutorial_help():
    result = subprocess.run(
...
def test_cli_search_engine_help():
    result = subprocess.run(
...
def test_search_args_default_to_bundled_database():
    class Args:
        db = None
...
def test_cli_search_debug_json_includes_metadata():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(embeddings, "generate_embeddings", return_value=[[0.0] * 256]):
            ...
            assert "mode" in output["metadata"]
            assert "results" in output

def test_cli_search_debug_text_includes_metadata():
    from rag import embeddings
    from rag.cli import cmd_search
    from unittest.mock import MagicMock
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(embeddings, "generate_embeddings", return_value=[[0.0] * 256]):
            ...
            assert "search_mode:" in output
            assert "vector_available:" in output

def test_cli_search_without_debug_no_metadata():
    from rag import embeddings
    from rag.cli import cmd_search
    from unittest.mock import MagicMock
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(embeddings, "generate_embeddings", return_value=[[0.0] * 256]):
            ...
            assert "search_mode:" not in output
            assert "vector_available:" not in output

def test_cli_search_debug_json_includes_ranking_signals():
    from rag import embeddings
    from rag.cli import cmd_search
    from unittest.mock import MagicMock
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "results" in output
        assert output["results"], "should have at least one result"
        top = output["results"][0]
        assert "ranking_signals" in top
        assert isinstance(top["ranking_signals"], list)
        assert top["ranking_signals"], "top result should have ranking signals"
        sig = top["ranking_signals"][0]
        assert "name" in sig
        assert "weight" in sig
        assert "details" in sig

def test_cli_search_debug_text_includes_signal_summary():
    from rag.cli import cmd_search
    from unittest.mock import MagicMock
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "signals:" in output, "debug text output should include a signals summary line"

def test_cli_search_default_text_omits_signals():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "signals:" not in output, "default text output must not print signal payload"


def test_how_to_query_prefers_tutorial_result():
    with tempfile.TemporaryDirectory() as tmp:
        docs = Path(tmp) / "docs"
        classes = docs / "classes"
        tutorials = docs / "tutorials"
        ...
        assert results
        assert results[0].doc_type == "tutorial"
        assert results[0].path == "tutorials/scene_tree.md"


def test_build_database_importable_from_indexer():
    from rag.indexer import build_database as indexer_fn
    from rag.store import build_database as store_fn
    assert indexer_fn is store_fn


def test_require_db_returns_path_when_exists():
...
def test_require_db_exits_when_missing():
    from rag.cli import _require_db
    from unittest.mock import MagicMock
...
def test_run_search_calls_search_database():
    from rag.cli import _run_search
    from unittest.mock import MagicMock, patch

    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_run_search_db(tmp)
        args = MagicMock()
        args.db = str(db_path)
        args.query = "timer"
        args.limit = 3
        ...
        with patch("rag.cli.search_database") as mock_search:
            ...
            mock_search.assert_called_once()
            call_kwargs = mock_search.call_args
            assert call_kwargs.kwargs.get("doc_types") == None
            assert call_kwargs.kwargs.get("addon") == None

def test_run_search_passes_doc_types():
    from rag.cli import _run_search
    from unittest.mock import MagicMock, patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("rag.cli.search_database") as mock_search:
            ...
            call_kwargs = mock_search.call_args
            assert call_kwargs.kwargs.get("doc_types") == ["class"]

def test_run_search_passes_addon():
    from rag.cli import _run_search
    from unittest.mock import MagicMock, patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("rag.cli.search_database") as mock_search:
            ...
            call_kwargs = mock_search.call_args
            assert call_kwargs.kwargs.get("doc_types") == ["addon"]
            assert call_kwargs.kwargs.get("addon") == "myaddon"

def test_run_search_respects_no_expand():
    from rag.cli import _run_search
    from unittest.mock import MagicMock, patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("rag.cli.search_database") as mock_search:
            ...
            _run_search(args)
            call_kwargs = mock_search.call_args
            assert call_kwargs.kwargs.get("expand_graph") == False

def test_run_search_debug_uses_metadata_variant():
    from rag.cli import _run_search
    from unittest.mock import MagicMock, patch
...
def _build_symbol_recall_db(tmp):
    docs = Path(tmp) / "docs"
    classes = docs / "classes"
    classes.mkdir(parents=True)
    (classes / "class_node.md").write_text(
...
def test_exact_symbol_match_records_signal():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_symbol_recall_db(tmp)
        results = search_database(db_path, "Node.add_child", limit=3, expand_graph=False)
        assert results
        top = results[0]
        assert top.symbol == "Node.add_child"
        names = [s.name for s in top.ranking_signals]
        assert "symbol_recall.exact" in names
        exact = next(s for s in top.ranking_signals if s.name == "symbol_recall.exact")
        assert exact.weight == 100.0

def test_suffix_symbol_match_records_signal():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert top.symbol == "Node.add_child"
        names = [s.name for s in top.ranking_signals]
        assert "symbol_recall.suffix" in names
        suffix = next(s for s in top.ranking_signals if s.name == "symbol_recall.suffix")
        assert suffix.weight == 80.0

def test_prefix_symbol_match_records_signal():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_symbol_recall_db(tmp)
        ...
        assert method, "Node.add_child should be found via prefix symbol match" is not None
        names = [s.name for s in method.ranking_signals]
        assert "symbol_recall.prefix" in names
        prefix = next(s for s in method.ranking_signals if s.name == "symbol_recall.prefix")
        assert prefix.weight == 40.0

def test_alias_derived_match_sets_alias_detail():
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_symbol_recall_db(tmp)
        results = search_database(
        ...
        assert exact is not None, "alias-derived exact match should still record symbol_recall.exact"
        assert exact.details.get("alias_derived"), "alias-derived match should set details.alias_derived=True"


def test_fts_fallback_records_bm25_signal():
    from rag import embeddings
    from unittest.mock import patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert has_fts, "FTS fallback should record fts.bm25 signal"

def test_hybrid_rrf_records_signal_when_vector_available():
    from rag import embeddings
    from unittest.mock import patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert has_rrf, "Hybrid mode should record hybrid.rrf signal"

def test_symbol_recall_preserves_prior_rrf_signal():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        top = results[0]
        assert top.symbol == "Timer.is_stopped"
        names = {s.name for s in top.ranking_signals}
        assert "hybrid.rrf" in names, "prior RRF signal must survive symbol-recall replacement"
        assert "symbol_recall.exact" in names


def test_new_graph_chunk_records_expansion_signal():
    from rag import embeddings
    from unittest.mock import patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert obj, "Object should be reached via graph expansion" is not None
        names = [s.name for s in obj.ranking_signals]
        assert "graph.expansion" in names
        graph_sig = next(s for s in obj.ranking_signals if s.name == "graph.expansion")
        assert graph_sig.details.get("relation") == "inherits"
        assert graph_sig.details.get("distance") == 1

def test_graph_expansion_preserves_prior_fts_signal():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert summary, "Node class_summary should be in results" is not None
        names = {s.name for s in summary.ranking_signals}
        assert "fts.bm25" in names, "prior FTS signal must survive graph expansion"
        assert "graph.expansion" in names, "graph expansion should append its own signal"
        graph_sig = next(s for s in summary.ranking_signals if s.name == "graph.expansion")
        assert graph_sig.details.get("metadata_only", "existing-chunk branch should set metadata_only=True")


def _build_ranking_signal_db(tmp, with_vectors=False):
    from rag import embeddings
    docs = Path(tmp) / "docs"
    classes = docs / "classes"
...
def test_core_signal_families_observable_via_search_database():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        db_path = _build_ranking_signal_db(tmp)
        ...
        hybrid_db = _build_ranking_signal_db(tmp, with_vectors=True)
    ...
    assert not missing, f"missing signal families: {sorted(missing)}"

def test_addon_intent_signal_observable_via_rerank():
...
def _build_chunk_relations_db(docs_text_map):
    import sqlite3
    import tempfile
...
def test_inherits_relation_created_for_class_summary():
    from rag.relations import build_chunk_relations
    conn = _build_chunk_relations_db({
    ...
    assert row, "inherits relation should exist" is not None
    assert row["relation"] == "inherits"
    assert row["weight"] == pytest.approx(0.8)


def test_node_inherits_object_reaches_object_class_summary():
    from rag import embeddings
    from unittest.mock import patch
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        obj = next((r for r in results if r.symbol == "Object"), None)
        assert obj, "Object class_summary should be reached via inherits traversal" is not None
        rank = results.index(obj) + 1
        assert rank <= 5, f"Object rank {rank} should be <= 5"
        names = [s.name for s in obj.ranking_signals]
        assert "graph.inherits" in names


def _build_db_with_noise(tmp):
    from rag import embeddings
    from unittest.mock import patch
    from rag.indexer import build_database
...
def test_inheritance_recall_pulls_class_summary_into_top_k():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        db_path = _build_db_with_noise(tmp)
        results = search_database(
            db_path, "Node inherits Object", limit=5, expand_graph=True,
        )
    symbols = [r.symbol for r in results]
    assert "Node" in symbols, "Node class_summary should be recalled into top-5 by step 3.5"
    assert "Object" in symbols, "Object class_summary should be reached (via step 3.5 recall or inherits traversal"
    node = next(r for r in results if r.symbol == "Node")
    names = [s.name for s in node.ranking_signals]
    assert "inheritance_recall.class_summary" in names, "Node must carry the inheritance_recall.class_summary signal (D3"

def test_non_inheritance_query_does_not_trigger_inheritance_recall():
    ...
    for r in results:
        for sig in r.ranking_signals:
            assert sig.name != "inheritance_recall.class_summary", "inheritance_recall signal must not fire for non-inheritance queries (D4 gating"
