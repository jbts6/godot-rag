...
- [x] 2.3 Extract database indexing/build logic from `store.py` into an indexing module and delegate `rag.store.build_database` to it.
- [x] 2.4 Extract search execution, vector fallback, FTS fallback, RRF fusion, graph expansion, snippet extraction, and result mapping from `store.py` into a search module and delegate `rag.store.search_database*` to it.
- [x] 2.5 Run focused search tests after the store split.
