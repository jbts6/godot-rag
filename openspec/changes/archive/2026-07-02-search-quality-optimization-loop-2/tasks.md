...
- [x] 1.1 在 `rst2md/rag/query_rewrite.py` 的 `expand_query_variants` 增加点号拆分逻辑：query 匹配 `^[A-Z][A-Za-z0-9_]*\.[A-Za-z_][A-Za-z0-9_]*\(?\s*$` 时，追加方法后缀变体（最后一个 `.` 之后的部分，去掉末尾 `()`）。保留原 query 作为第一个变体
...
- [x] 1.3 验证 `_symbol_candidates`（`query_plan.py:30`）对 `["Node.connect", "connect"]` 输出两个去重候选 `("Node.connect", "connect")`
- [x] 1.4 跑 `uv run pytest -q rst2md/tests/test_searcher_module.py rst2md/tests/test_rag_search.py` 确认 A 段单元测试通过且无回归
- [x] 1.5 跑 `uv run godot-rag eval-search`，把输出存为 `docs/search-quality/loop-2-stage-A.json`
- [x] 1.6 验证 `ResourceLoader.load` 与 `Node.connect` 命中 rank ≤3；symbol 类 hit@5 80%→90%+；32 个原通过查询 hit@5 不低于 89.5%。若有回归，记录是哪个查询、分析是否需要给方法后缀加最小长度守卫
...
- [x] 2.9 验证 `scene tree tutorial` 与 `how to use scene tree nodes` 命中 rank ≤5；tutorial 类 hit@5 71%→100%；A 段通过的查询不回归
...
- [x] 3.3 在 `rst2md/rag/query_plan.py` 加 `_inheritance_intent(query: str) -> bool`：检测 `inherits`、`extends`、`subclass of`、`parent class`、`derived from` 关键词（大小写不敏感，整词匹配）
- [x] 3.4 在 `QueryPlan` dataclass 加 `inheritance_intent: bool` 字段；`build_query_plan` 调用 `_inheritance_intent` 填充
...
- [x] 4.5 准备 verify 阶段所需材料：design.md 中三个 Open Questions 的答案（factor 最终值、Inherits 行格式确认、A 段回归检查结果）
